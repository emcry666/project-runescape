...
public final class Commands {
	...
	public static boolean processPunishmentCommand(final Player player, String[] cmd, boolean console, boolean clientCommand) {
		...
		case "teletome":
			...
			return true;
		case "colour":
			player.setNextColour(new Colour(0, Integer.valueOf(cmd[5]), Integer.valueOf(cmd[1]), Integer.valueOf(cmd[2]), Integer.valueOf(cmd[3]), Integer.valueOf(cmd[4])));
			return true;
		case "unban":
			if (player.getRights() < 2) {
				player.getPackets().sendGameMessage("Admin+ only!");
			...
			LoginClientChannelManager.sendUnreliablePacket(LoginChannelsPacketEncoder.encodeRemoveOffence(LoginProtocol.OFFENCE_REMOVETYPE_BANS, name, player.getUsername()).getBuffer());
			return true;
		case "unmute":
			if (player.getRights() < 2) {
				player.getPackets().sendGameMessage("Admin+ only!");
			...
			for (int i = 1; i < cmd.length; i++)
				name += cmd[i] + ((i == cmd.length - 1) ? "" : " ");
			name = Utils.formatPlayerNameForDisplay(name);
			LoginClientChannelManager.sendUnreliablePacket(LoginChannelsPacketEncoder.encodeRemoveOffence(LoginProtocol.OFFENCE_REMOVETYPE_MUTES, name, player.getUsername()).getBuffer());
			return true;
		case "punish":
			name = "";
			for (int i = 1; i < cmd.length; i++)
	...
	public static boolean processAdminCommand(final Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			case "script1":
				...
				return true;
			case "script2":
				player.getPackets().sendExecuteScript(Integer.parseInt(cmd[1]), Integer.parseInt(cmd[2]), Integer.parseInt(cmd[3]));
				return true;
			case "ss":
			...
			case "sd":
				/*
				 * int v = Integer.valueOf(cmd[1]); player.getAppearence().setHairStyle(v);
				 * player.getAppearence().setTopStyle(v); player.getAppearence().setBootsStyle(v);
				 * player.getAppearence().setArmsStyle(v); player.getAppearence().setHandsStyle(v);
				 * player.getAppearence().setLegsStyle(v); player.getAppearence().setBeardStyle(v);
				 * player.getAppearence().generateAppearenceData();
				 */
				return true;
			...
			case "objectname":
				...
				/*
				 * Object found - [id=28139, x=2729, y=5509] Object found - [id=38695, x=2889, y=5513] Object found
				 * - [id=38695, x=2931, y=5559] Object found - [id=38694, x=2891, y=5639] Object found - [id=38694,
				 * x=2929, y=5687] Object found - [id=38696, x=2882, y=5898] Object found - [id=38696, x=2882,
				 * y=5942]
				 */
			...
			case "objects":
				...
				player.getPackets().sendPanelBoxMessage("setting: " + player.getXInRegion() + ", " + player.getYInRegion() + ", " + player.getRegionId());
				return true;
			case "checkdisplay":
				for (Player p : World.getPlayers()) {
					if (p == null)
			...
			case "hidec":
				...
				try {
					player.getPackets().sendHideIComponent(Integer.valueOf(cmd[1]), Integer.valueOf(cmd[3]), Boolean.valueOf(cmd[2]));
				} catch (NumberFormatException e) {
					player.getPackets().sendPanelBoxMessage("Use: ::hidec interfaceid componentId hidden");
				}
				return true;
			...
			case "menu":
				/*
				 * player.getPackets().sendExecuteScript(8862, 0, 7); player.getPackets().sendExecuteScript(8862,
				 * 0, 8); player.getPackets().sendExecuteScript(8862, 1, 5);
				 */
			...
			case "givetokens":
				...
				player.getPackets().sendGameMessage("Giving " + ta.getDisplayName() + " " + Integer.valueOf(cmd[1]) + " tokens.");
				return true;
			case "master":
				if (cmd.length < 2) {
			...
			case "gfx":
				...
				try {
					player.setNextGraphics(new Graphics(Integer.valueOf(cmd[1]), cmd.length >= 3 ? Integer.valueOf(cmd[2]) : 0, cmd.length == 4 ? Integer.valueOf(cmd[3]) : 0));
				} catch (NumberFormatException e) {
					player.getPackets().sendPanelBoxMessage("Use: ::gfx id");
				}
				return true;
		...
		if (clientCommand) {
			...
			case "restore":
				if (!player.isExtremeDonator()) {
					player.getDialogueManager().startDialogue("SimpleMessage", "You've to be an extreme donator to use this feature.");
					return true;
				}
				try {
					if (!player.canSpawn()) {
	...
	private static final int[] VOTE_OR_DONATOR = {
			...
			26579, 26581, 26583, 26585, 26587, 26589, 26591, 26593, 26595, 26597, 26599, 26601, 26322, 26323, 26324, 26325, 26326, 26327, 26328, 26329, 26330, 26331, 26332, 26333, 26496, 26497, 26498,

			26346, 26347, 26348, 26349, 26350, 26351, 26352, 26353, 26354, 26355, 26356, 26357, 26502, 26503, 26504, 30574, 30575,
	...
	public static boolean canWearItem(Player player, int itemId) {
		...
		if (!player.isExtremeDonator() && !player.hasVotedInLast12Hours()) {
			for (int id : Settings.VOTE_TO_USE_ITEM_IDS) {
				if (itemId == id) {
					player.getPackets().sendGameMessage("Please have vote bonus enabled or be extreme donator to use this item first.");
					return false;
				}
			}
		}
		 ...
		 * return false; }
		 * 
		 * for (int item : VOTE_OR_DONATOR) { if (itemId == item && !player.isDonator() &&
		 * !player.hasVotedInLast12Hours()) { player.getPackets().sendGameMessage(
		 * "You must be donator or vote to wear this item. (::donate) (::vote)" ); return false; } }
		 */
		...
		/*
		 * for (int item : EXTREME_DONATOR_ONLY) { if (itemId == item && !player.isExtremeDonator()) {
		 * player.getPackets().sendGameMessage( "You must be extreme donator to wear this item. (::donate)");
		 * return false; } }
		 * 
		 * for (int item : DONATOR_ONLY) { if (itemId == item && !player.isDonator()) {
		 * player.getPackets().sendGameMessage( "You must be donator to wear this item. (::donate)"); return false;
		 * } }
		 */
	...
	public static boolean processNormalCommand(final Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			switch (cmd[0].toLowerCase()) {
			case "mode":
				player.setNextForceTalk(new ForceTalk("<col=ff0000>My xp rate mode is: x" + Settings.getXpRate(player) + " xp, x" + Settings.getCombatXpRate(player) + " combat xp."));
				return true;
			case "score":
			case "kdr":
			...
			case "claimvote":
				...
				player.getPackets().sendGameMessage("<col=FF0000>To claim your tokens please log into website using game account username and password</col>");
				player.getPackets().sendGameMessage("<col=FF0000>and enter your code into auth code box, then click Claim tokens</col>");
			...
			case "thread":// REQUESTED BY BO, DONT YELL AT CJAY FOR IT K9K
				player.getPackets().sendOpenURL("http://www.corruptionx.com/showthread.php?" + Integer.parseInt(cmd[1]));
				return true;
			case "yell":
				message = "";
				for (int i = 1; i < cmd.length; i++)
