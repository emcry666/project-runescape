...
public class Bank implements Serializable {
	...
	public void openPin(final int pinType) {
		if (pin != -1 && recoveryTime >= System.currentTimeMillis() && player.getTemporaryAttributtes().get(Key.RECOVERY_VERIFIED) == null) {// Pin
			// is
			// set
			// and
			// there
			...
			player.getDialogueManager().startDialogue("PinMessageD", "You must wait " + (disableTime / 60000) + " minutes before being able to attempt your PIN once more.");
			return;
		}
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			public void start() {
				...
				player.getPackets().sendIComponentText(13, 27, pinType == 1 ? "ENTER IDENTICAL PIN" : recoveryTime >= System.currentTimeMillis() ? "Your pin will be removed in " + ((recoveryTime - System.currentTimeMillis()) / 86400000) + " days." : "ENTER YOUR PIN.");
		...
		if (count == 3) {
			if (!isValidInput(pin, pin_type)) {
				if (failedAttempts++ == 3) {
					...
					// minutes.
					player.getDialogueManager().startDialogue("PinMessageD", "All bank attempts have been disabled for the next <col=7E2217>FIVE MINUTES.");
					return;
				}
	...
	public void sendBoxInterItems() {
		player.getPackets().sendInterSetItemsOptionsScript(11, 1, 93, 6, 5, "Deposit-1", "Deposit-5", "Deposit-10", "Deposit-All", "Deposit-X", "Examine");
