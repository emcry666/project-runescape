...
public final class WorldPacketsDecoder extends Decoder {
	...
	private final static int OBJECT_CLICK2_PACKET = 90;
	...
	public static void decodeLogicPacket(final Player player, LogicPacket packet) {
		...
		if (opcode == WALKING_PACKET || opcode == MINI_WALKING_PACKET) {
			...
			case 1140:
			case 234:
				if ((interfaceId == 1110 && componentId == 16) || (interfaceId == 1140 && componentId == 151) || (interfaceId == 234 && componentId == 10))
				...
				synchronized (manager) {
					 ...
					 * player.getPackets().sendGameMessage( "Clans can't have over 500 members."); return; }
					 */
				}
			...
			case 1165:
				/*
				 * if (componentId == 3) { if (!player.getControllerManager().canAttack(npc)) {
				 * player.getInterfaceManager().closeInventory(); return; } else if (player.getAttackedBy() ==
				 * null) { player.getPackets().sendGameMessage(
				 * "You need to have a target in order to deploy a dreadnip.");
				 * player.getInterfaceManager().closeInventory(); return; } player.getInventory().deleteItem(22370,
				 * 1); Dreadnip dread = new Dreadnip(player, Utils.getFreeTile(player, 2), -1, true);
				...
				public void run() {
					...
					if (World.removeGroundItem(player, item)) {
						try {
							...
							writer.write("[" + dateFormat.format(cal.getTime()) + ", IP: " + player.getSession().getIP() + "] has picked up item [ id: " + item.getId() + ", amount: " + item.getAmount() + " ] originally owned to " + (item.getOwner() == null ? "no owner" : item.getOwner()));
							writer.newLine();
							writer.flush();
							writer.close();
	...
	public void processPackets(final int opcode, InputStream stream) throws ClassNotFoundException, IOException {
		...
		if (opcode == PING_PACKET) {
			...
			/*
			 * player.getInterfaceManager().removeAll(); player.getInterfaceManager().sendInterfaces();
			...
			boolean clicked;
			/*
			 * BotDetector.readMousePosition(player, x, y, opcode); System.err.println(x + ", " + y);
			 */
			...
			int fromItemId = stream.readShortLE();
			...
			if ((fromInterfaceId == Inventory.INVENTORY_INTERFACE || fromInterfaceId == Inventory.INVENTORY_INTERFACE_2) && (toInterfaceId == Inventory.INVENTORY_INTERFACE || toInterfaceId == Inventory.INVENTORY_INTERFACE_2)) {
			...
			} else if (toInterfaceId == 1430 && (toComponentId >= 55 && toComponentId <= 229) || (toInterfaceId == 1436 && (toComponentId >= 25 && toComponentId <= 194))) { // ability
				// bar
				// switch shortcut
				if (fromInterfaceId == 1430 && (fromComponentId >= 55 && fromComponentId <= 229) || (fromInterfaceId == 1436 && (fromComponentId >= 25 && fromComponentId <= 194)))
				...
				// item shortcut inv
					...
					if (ActionBar.getAbilityData(usingDefenceAbilities ? ActionBar.DEFENCE_ABILITY_SHORTCUT : ActionBar.HEAL_ABILITY_SHORTCUT, fromSlot) == null) // fake
						// spell
						return;
			...
			} else if (fromInterfaceId == 1430 && (fromComponentId >= 55 && fromComponentId <= 229) || (fromInterfaceId == 1436 && (fromComponentId >= 25 && fromComponentId <= 194))) { // ability
				// bar
				// drag
			...
			try {
				...
				Calendar cal = Calendar.getInstance();
				BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_PATH + player.getUsername() + ".txt", true));
				writer.write("[" + dateFormat.format(cal.getTime()) + ", IP: " + player.getSession().getIP() + "]: to " + target + " " + message);
				writer.newLine();
				writer.flush();
				writer.close();
			} catch (IOException e) {
			...
			if (chatType == 1) {
				...
				if (player.getControlerManager().getControler() instanceof DungeonController) {
					for (Player party : player.getDungManager().getParty().getTeam()) {
						/*
						 * if (player.getLocalPlayerUpdate().getLocalPlayers()[ party.getIndex()] == null ||
						 * party.getLocalPlayerUpdate().getLocalPlayers()[player .getIndex()] == null) {
						 * party.getPackets().sendGameMessage(player. getDisplayName() + ":<col=7fa9ff> " +
						 * message); }
						 */
			...
			else if (path.contains("title.ws") || path.toLowerCase().contains("squeal") || path.toLowerCase().contains("treasure hunter"))
