...
public class ButtonHandler {
	...
	public static void handleButtons(final Player player, InputStream stream, final int packetId) throws ClassNotFoundException, IOException {
		...
		System.out.println(interfaceId + ", " + componentId + ", " + slotId);
		if (Settings.DEBUG)
			System.out.println("Action Button: InterfaceId = " + interfaceId + ", ChildId = " + componentId + ", SlotId = " + slotId + ", SlotId2 = " + itemId);
		...
		if (player.isDead() || player.isLocked()) {
			if (player.getCutscenesManager().hasCutscene() && player.getCutscenesManager().getCurrent().allowSkipCutscene() && interfaceId == 1477 && componentId == 21 && packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
		...
		if (interfaceId == 1371) { // skill dialogue rs3
			...
			if (componentId == 9) {
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					...
					/*
					 * player.getPackets().sendIComponentSettings(1422, 66, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 67, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 68, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 69, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 70, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 71, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 72, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 73, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 74, 2, 2, 2);
					 * player.getPackets().sendIComponentSettings(1422, 75, 2, 2, 2);
					 */
					 ...
					 * 
					 * @Override public void run() { player.getInterfaceManager().sendGameMapInterface();
			...
			if (componentId == 1)
				player.getActionbar().useAbility(player.getCombatDefinitions().onStrengthMenu() ? new StrengthAbilityShortcut(slotId) : new MeleeAbilityShortcut(slotId), packetId);
			else if (componentId == 5 && (slotId == 6 || slotId == 7))
				player.getCombatDefinitions().setStrengthMenu(slotId == 7 ? 1 : 0);
			player.getCombatDefinitions().setStrengthMenu(slotId == 7 ? 1 : 0);
		} else if (interfaceId == 1449) {
			if (componentId == 1)
				player.getActionbar().useAbility(player.getCombatDefinitions().onDefenceMenu() ? new DefenceAbilityShortcut(slotId) : new HealAbilityShortcut(slotId), packetId);
			...
			if (componentId == 5) {// hero f1
				...
				case WorldPacketsDecoder.ACTION_BUTTON5_PACKET:
					...
					break;
				case WorldPacketsDecoder.ACTION_BUTTON9_PACKET:// extras
					player.getInterfaceManager().openMenu(7, player.getSubMenus()[7] + 1);
					break;
				case WorldPacketsDecoder.ACTION_BUTTON6_PACKET:// rune metrics
					player.getInterfaceManager().openMenu(8, player.getSubMenus()[8] + 1); // rune metrics
					break;
				}
			} else if (componentId == 7)
				player.getInterfaceManager().openMenu(0, player.getSubMenus()[0] + 1);
			...
			if (componentId == 42) {// game settings
		...
		} else if (interfaceId == 1312 || interfaceId == 668 || interfaceId == 737 || interfaceId == 1262 || interfaceId == 1292 || interfaceId == 793) {
			...
			if ((interfaceId == 1417 && (componentId == 11 || componentId == 18)) || (interfaceId == 34 && (componentId == 12 || componentId == 18))) {
				switch (packetId) {
				case WorldPacketsDecoder.ACTION_BUTTON1_PACKET:
			...
			} else if ((interfaceId == 1417 && (componentId >= 34 && componentId <= 52)) || (interfaceId == 34 && (componentId >= 31 && componentId <= 52))) {
				player.getNotes().colour(interfaceId == 1417 ? (componentId == 34 ? 0 : componentId == 40 ? 1 : componentId == 46 ? 3 : 2) : (componentId == 52 ? 0 : componentId == 45 ? 1 : componentId == 38 ? 3 : 2));
			...
			else if ((interfaceId == 187 && (componentId == 13 || componentId == 17)) || (interfaceId == 1416 && (componentId == 16 || componentId == 19)))
			...
			if ((interfaceId == 1458 && (componentId == 33)))
				player.getPrayer().delayUsePrayer(slotId, false);
			else if ((interfaceId == 1458 && (componentId == 10 || componentId == 14)) || (interfaceId == 1457 && (componentId == 21 || componentId == 31)))
				player.getPrayer().switchSettingQuickPrayer();
			...
			if ((componentId >= 17 && componentId <= 40) || componentId == 99 || componentId == 130 || componentId == 165)
			...
			} else if ((interfaceId == Inventory.INVENTORY_INTERFACE && componentId == 8) || (interfaceId == Inventory.INVENTORY_INTERFACE_2 && componentId == 15)) {
			...
			if ((interfaceId == Inventory.INVENTORY_INTERFACE && componentId == 57) || (interfaceId == Inventory.INVENTORY_INTERFACE_2 && componentId == 24)) {
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) {
			...
			if (componentId == 30) {
				...
				player.getPackets().sendGameMessage("This functionality has yet to be added.");
			}
			...
			if (componentId == 16) {
				...
				player.getPackets().sendInterfaceMessage(interfaceId, componentId, 0, slotId, ItemExamines.getExamine(item) + (ItemConstants.isTradeable(item) ? "<br>GE guide price: " + GrandExchange.getPrice(item.getId()) + " gp" : ""));
			}
			...
			// legacy
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON4_PACKET)
					...
					player.switchProfanityFilter();
					player.getPackets().sendGameMessage("Your profanity filter is now " + (player.isFilteringProfanity() ? "on" : "off") + " .");
				}
			} else if (componentId == 296)
				player.getPlayerExamineManager().openExamineSettings();
	...
	public static boolean sendRemove(Player player, int slotId, boolean toBank) {
		...
		if (toBank) {
			...
			if (slots == null && !player.getBank().hasBankSpace())
				return false;
			player.getBank().addItem(item.getId(), slots == null ? player.getBank().getCurrentTab() : slots[0], item.getAmount(), true);
	...
	public static boolean wear(Player player, int slotId, int itemId, boolean fromBank) {
		...
		if (isTwoHandedWeapon && player.getEquipment().getItem(Equipment.SLOT_SHIELD) != null && !player.getInventory().hasFreeSlots()) {
	...
	public static void sendItemsKeptOnDeath(Player player, boolean wilderness) {
		...
		text.append("<br>").append("The maximum this can be").append("<br>").append("boosted to is 5.").append("<br>").append("<br>");
		if (wilderness) {
			...
			text.append("Gravestone:").append("<br>").append(ClientScriptMap.getMap(1099).getStringValue(player.getGraveStone())).append("<br>").append("<br>").append("Initial duration:").append("<br>").append(minutes + ":" + (seconds < 10 ? "0" : "") + seconds);
		}
		text.append("<br>").append("<br>").append("Carried wealth:").append("<br>").append(carriedWealth > Integer.MAX_VALUE ? "Too high!" : Utils.getFormattedNumber((int) carriedWealth)).append("<br>").append("<br>").append("Risked wealth:").append("<br>").append(riskedWealth / 10 > Integer.MAX_VALUE ? "Too high!" : Utils.getFormattedNumber((int) riskedWealth / 10)).append("<br>").append("<br>");
		if (wilderness) {
