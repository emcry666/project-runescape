...
public class KalphiteKing extends NPC {
	...
	/*
	 * What was wrong and what has been done (x) Asked this to both streamers
	 * and a couple of friends who has done kk a lot 3 attacks then special(x)
	 * start at ANY colour rather then always melee(x) dig resets the atks to
	 * special(x)
	 * 
	...
	public void sendDeath(final Entity source) {
		if (!usedImmortality && Utils.random(100) < 15) { // 15% chance using
			// imortality :p
	...
	public void processNPC() {
		...
		if (canSwitch())
			switchPhase();
		if ((getHPPercentage() < 75 && spawnCount < 1) || (getHPPercentage() < 25 && spawnCount < 2))
			battleCry();
		if (auraTicks == 1) {
			auraTicks = 0;
			if (!attackedSomeone)
		...
		if (!isCantInteract() && this.getCombat().getCombatDelay() == 0) {
			...
			for (Entity t : getPossibleTargets()) {
				if (Utils.colides(this, t)) {
					t.applyHit(new Hit(this, Utils.random(128) + 1, HitLook.REGULAR_DAMAGE));
				}
			}
		}
	...
	private void battleCry() {
		...
		for (Entity t : getPossibleTargets()) { // battle cry
			if (Utils.isOnRange(this, t, 3)) { // if nearby when he uses it u
				// get dmg
				t.applyHit(new Hit(this, Utils.random(128) + 1, HitLook.REGULAR_DAMAGE));
			}
		}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (part1) {
					...
					// if instance spawned be sure that the player didnt leave
					// or the boss would tp outside
					// if npc dont make it tp under to be sure it doesnt bug
			...
			public void run() {
				...
				for (Entity target : getPossibleTargets()) {
					if (Utils.colides(KalphiteKing.this, target)) {
						target.applyHit(new Hit(KalphiteKing.this, Utils.random(300, 500), HitLook.REGULAR_DAMAGE));
