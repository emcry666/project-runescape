...
public class KalphiteKingCombat extends CombatScript {
	...
	private void sendStandardAttack(final NPC npc, Entity target) {
		...
		case 16697: // melee //DONE
			...
			if (Utils.random(10) == 0) { // blind
				...
				delayHit(npc, 0, target, getMeleeHit(npc, getMaxHit(npc, 128, NPCCombatDefinitions.MELEE, target)));
			}
			break;
		case 16698: // mage yelllow-green orb(mage) //DONE
			...
			for (Entity t : npc.getPossibleTargets()) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					@Override
					public void run() {
						World.sendGraphics(npc, new Graphics(3743), tile); // TODO
						// correct
						// gfx
						// here
						// for
							...
							public void run() {
								...
								for (Entity t : npc.getPossibleTargets()) {
									if (t.withinDistance(tile, 2)) {
										if (twoOrbs) {
											t.applyHit(new Hit(npc, Utils.random(250) + 1, HitLook.MAGIC_DAMAGE));
											t.applyHit(new Hit(npc, Utils.random(250) + 1, HitLook.MAGIC_DAMAGE));
										} else {
											if (Utils.random(2) == 1)
												t.applyHit(new Hit(npc, Utils.random(300) + 1, HitLook.MAGIC_DAMAGE));
										}
									}
								}
							}
		...
		case 16699: // range //DONE
			...
			for (Entity t : npc.getPossibleTargets()) {
				...
				if (!king.isUseStunDartNext() && Utils.random(10) == 0) { // blind
					...
					delayHit(npc, delay, t, getRangeHit(npc, getMaxHit(npc, 120, NPCCombatDefinitions.RANGE, t)));
			...
			if (specificSpecial) { // more common
				...
				case 16697: // melee quake (melee) //DONE
					...
					for (Entity t : npc.getPossibleTargets()) {
						if (Utils.isOnRange(king, t, 3)) {
							Hit hit = getMeleeHit(npc, getMaxHit(npc, 250, NPCCombatDefinitions.MELEE, t));
				...
				case 16698: // mage blue orb(mage) //DONE
					...
					for (Entity t : npc.getPossibleTargets()) {
						...
						int delay = Utils.projectileTimeToCycles(projectile.getEndTime()) - 1;
						delayHit(npc, delay, t, getMagicHit(npc, getMaxHit(npc, 250, NPCCombatDefinitions.MAGE, t)));
						WorldTasksManager.schedule(new WorldTask() {
					...
					return 17;
				// break;
				case 16699: // range //incendiary shot (range) / stun dart
					// (after incendiary shot / phase changing to range)
					...
					for (Entity t : list) {
						if (c++ == 3)
							break;
						delayHit(npc, 0, t, getRangeHit(npc, getMaxHit(npc, 400, NPCCombatDefinitions.RANGE, t)));
				...
				if ((attack == 0 && king.getId() == 16698) || (attack == 1 && king.getId() == 16699))
					attack = 2;
				...
				case 0: // Beetlejuice (melee + range) //DONE
					...
					if (target instanceof Player) {
						...
						((Player) target).setNextAnimation(new Animation(-1)); // to
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							if (king.getCombat().getTarget() != target)
								return;
							target.applyHit(new Hit(king, 100, HitLook.MELEE_DAMAGE));
						}
					}, 11);
					return 12; // add more delay for this one or else it will
				// remove ability
				// break;
				case 1:// charge Charge (melee + mage)
					final byte[] dirs = Utils.getDirection(npc.getDirection()); // gets
					// the
					// di
					// x/y
					// for
					...
					for (distance = 1; distance < 10; distance++) {
						...
						if (!World.isFloorFree(nextTile.getPlane(), nextTile.getX(), nextTile.getY(), king.getSize()))
							break; // found last tile(as cant move this means
						// the one before this)
						lastTile = nextTile;
					}
									...
									if (Utils.colides(kingTile, t, king.getSize(), 1)) {
										...
										for (int thisStep = 1; thisStep <= leftSteps; thisStep++) {
											...
											if (!World.isFloorFree(nextTile.getPlane(), nextTile.getX(), nextTile.getY()))
												break; // found last tile(as
														// cant move this means
														// the one before this)
											lastTileForP = nextTile;
											stepCount = thisStep;
										}
										if (lastTileForP == null) // this player
											// didnt get
											// caught by
											// 1step
											continue;
					...
					return 17;
				/*
				 * case 2: //dig //DONE // dig isnt a special atk. its a normal
				 * atk. king.dig(target); break;
				 */
