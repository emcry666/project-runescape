...
public class CockatriceCombat extends CombatScript {
	...
	if (!Slayer.hasReflectiveEquipment(target)) {
	    ...
	    int randomSkill = /*Combat.COMBAT_SKILLS[Utils.random(Combat.COMBAT_SKILLS.length)]*/1;
	    ...
	    delayHit(npc, endTime, target, getMagicHit(npc, getMaxHit(npc, 200, npc.getAttackStyle(), target)));
