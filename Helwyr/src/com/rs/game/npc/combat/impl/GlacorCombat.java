...
public class GlacorCombat extends CombatScript {
		...
		if (target instanceof Player) {
			...
			if (glacor.getEffect() == 1)
				player.getPrayer().drainPrayer(
						(int) (player.getPrayer().getPrayerpoints() * .1));
			...
			case 4:
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					@Override
					public void run() {
						for (final Entity e : npc.getPossibleTargets(false,
								true)) {
							if (e instanceof Player) {
								final Player player = (Player) e;
								if (player.withinDistance(tile, 0))
									player.applyHit(new Hit(npc, player
											.getHitpoints() / 2,
											HitLook.RANGE_DAMAGE));
								player.getPackets().sendGraphics(
										new Graphics(2315), tile);
							}
						}
					}
				}, 3);
	...
	private void sendDistancedAttack(Glacor npc, final Entity target) {
		final boolean isRangedAttack = npc.isRangeAttack();
		if (isRangedAttack) {
			delayHit(
					npc,
					2,
