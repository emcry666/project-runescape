...
public class NexCombat extends CombatScript {
	...
	private void sendIceBarrage(Nex nex) {
		...
		for (final Entity possibleTarget : nex.getPossibleTargets()) {
			...
			int damage = getMaxHit(nex, 500, NPCCombatDefinitions.MAGE, possibleTarget);
		...
		if (players.size() > 0) {
			...
			for (int x = -1; x <= 1; x++) {
				for (int y = -1; y <= 1; y++) {
					...
					if (World.isTileFree(0, tile.getX(), tile.getY(),
							(object.getDefinitions().getSizeX() + object.getDefinitions().getSizeY()) / 2))
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							if (player.getX() == tile.getX() && player.getY() == tile.getY()) {
								player.getPackets()
								.sendGameMessage("The centre of the ice prison freezes you to the bone!");
								player.resetWalkSteps();
								player.applyHit(new Hit(nex, Utils.random(600), HitLook.REGULAR_DAMAGE));
							}
						}
					}, 8, 0);
				}
			...
			public void run() {
				for (int y = 1; y >= -1; y--) {
					for (int x = 1; x >= -1; x--) {
						...
						final WorldObject object = new WorldObject(57263, 10, 0, tile);
						if (tile != base && World.isTileFree(0, tile.getX(), tile.getY(),
								(object.getDefinitions().getSizeX() + object.getDefinitions().getSizeY()) / 2)) {
							for (Player player : ZarosGodwars.getPlayers()) {
								if (player.getX() == tile.getX() && player.getY() == tile.getY()) {
									...
									player.getPackets()
									.sendGameMessage("You've been injured and can't use "
											+ (player.getPrayer().isAncientCurses() ? "deflect curses"
													: "protection prayers ")
											+ "!");
									player.resetWalkSteps();
	...
	private void sendBloodAttack(final Nex nex, final Entity target) {
		final int damage = getMaxHit(nex, 500,NPCCombatDefinitions.MAGE, target);
		nex.setNextAnimation(new Animation(17414));
		nex.setNextGraphics(new Graphics(3375));
		...
		if (maximumAmount != 0) {
			for (int dir = 0; dir < dirs[0].length; dir++) {
				final WorldTile tile = new WorldTile(
						new WorldTile(target.getX() + dirs[0][dir], target.getY() + dirs[1][dir], target.getPlane()));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (count == 0) {
					...
					nex.setNextForceMovement(
							new ForceMovement(dir, 1, center, 3, index == 3 ? 1 : index == 2 ? 0 : index == 1 ? 3 : 2));
					for (Entity entity : nex.calculatePossibleTargets(center, dir, index == 0 || index == 2)) {
						if (entity instanceof Player) {
							...
							Hit hit = new Hit(nex, Utils.random(550), HitLook.REGULAR_DAMAGE);
							...
							player.setNextForceMovement(
									new ForceMovement(center, 2, index == 3 ? 3 : index == 2 ? 2 : index == 1 ? 1 : 0));
							WorldTasksManager.schedule(new WorldTask() {
					...
					if (!firstCall) {
						for (WorldTile tile : tiles) {
							World.sendGraphics(null, new Graphics(383),
									new WorldTile(tile.getX(), tile.getY(), tile.getPlane()));
							for (Entity t : nex.getPossibleTargets())
								if (t.getX() == tile.getX() && t.getY() == tile.getY()
								&& t.getPlane() == tile.getPlane())
									t.applyHit(new Hit(nex, Utils.random(550), HitLook.REGULAR_DAMAGE));
						}
						firstCall = true;
					} else {
	...
	private void sendMeleeAttack(Nex nex, Entity target) {
		...
		Hit hit = getMeleeHit(nex, getMaxHit(nex, 500, NPCCombatDefinitions.MELEE, target));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (ticks == 1) {
					...
					target.setNextForceMovement(new ForceMovement(nex, 2,
							Utils.getMoveDirection(nex.getCoordFaceX(target.getSize()) - target.getX(),
									nex.getCoordFaceY(target.getSize()) - target.getY())));
					nex.setTarget(target);
				} else if (ticks == 3) {
					target.setNextWorldTile(nex);
					target.getPackets().sendGameMessage("You've been injured and you cannot use "
							+ (target.getPrayer().isAncientCurses() ? "protective curses" : "protective prayers")
							+ "!");
				}
			}
		}, 0, 0);
	...
	private void sendRangeAttack(final Nex nex) {
		...
		for (final Entity t : nex.getPossibleTargets()) {
			...
			if (distance <= 10)
				damage = 200 - (distance * 400 / 11);
			else
				damage = 300 + Utils.random(275);
			Projectile projectile = World.sendProjectileNew(nex, t, 380, 25, 25, 32, 2, 0, 0);
			delayHit(nex, Utils.projectileTimeToCycles(projectile.getEndTime()) - 1, t,
					getRangeHit(nex, getMaxHit(nex, damage, NPCCombatDefinitions.RANGE, t)));
		}
	}
	...
	private void sendMagicAttack(final Nex nex, final boolean secondAttack) {
		for (final Entity t : nex.getPossibleTargets()) {
			...
			final int damage = getMaxHit(nex, 500, NPCCombatDefinitions.MAGE, t);
