...
public class JadinkoCombat extends CombatScript {
	...
	delayHit(npc, 2, target, getMagicHit(npc, getMaxHit(npc, 350, NPCCombatDefinitions.MAGE, target)));
    }

    private void meleeAttack(NPC npc, Entity target) {
	npc.setNextAnimation(new Animation(npc.getId() == 13820 ? 3009 : 3214));
	delayHit(npc, 0, target, getMeleeHit(npc, getMaxHit(npc, 350, NPCCombatDefinitions.MELEE, target)));
    }
}
