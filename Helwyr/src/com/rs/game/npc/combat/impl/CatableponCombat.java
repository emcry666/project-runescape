...
public class CatableponCombat extends CombatScript {
	...
	if (Utils.random(10) == 0 && target instanceof Player) {
		...
		delayHit(npc, 1, target, getMagicHit(npc, getMaxHit(npc, 275, NPCCombatDefinitions.MAGE, target)));
		return npc.getAttackSpeed();
	    }

	}
	delayHit(npc, 0, target, getMeleeHit(npc, getMaxHit(npc, 275, npc.getAttackStyle(), target)));
