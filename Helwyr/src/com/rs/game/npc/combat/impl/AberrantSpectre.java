...
public class AberrantSpectre extends CombatScript {
	...
	if (!Slayer.hasNosepeg(target)) {
		...
		targetPlayer.getPackets()
			.sendGameMessage("The smell of the abberrant spectre make you feel slightly weaker.");
	    }
	    delayHit(npc, 1, target, getMagicHit(npc, targetPlayer.getMaxHitpoints() / 10));
	    // TODO player emote hands on ears
	} else
	    delayHit(npc, 1, target, getMagicHit(npc, getMaxHit(npc, 310, npc.getAttackStyle(), target)));
