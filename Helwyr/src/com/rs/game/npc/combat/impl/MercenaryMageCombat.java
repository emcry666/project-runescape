...
public class MercenaryMageCombat extends CombatScript {
	...
	public int attack(final NPC npc, Entity target) {
		...
		if(attackStyle == 0) {
				...
				public void run() {
					for (Player player : World.getPlayers()) { // lets just loop
						...
						if (player.withinDistance(center, 3)) {
							...
							delayHit(npc, 0, player, new Hit(npc, Utils.random(600), HitLook.RANGE_DAMAGE));
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					for(Player player : World.getPlayers()) { //lets just loop all players for massive moves
						...
						World.sendGraphics(npc, new Graphics(369), player.getLastWorldTile());
						delayHit(npc, 0, player, new Hit(npc, Utils.random(420), HitLook.MAGIC_DAMAGE));
			...
			delayHit(npc, 2, target, getMagicHit(npc, getRandomMaxHit(npc, Utils.random(600), NPCCombatDefinitions.MAGE, target)));
			...
			delayHit(npc, 2, target, getMeleeHit(npc, getRandomMaxHit(npc, Utils.random(780), NPCCombatDefinitions.MELEE, target)));
		} else if (attackStyle == 6) {
			npc.setNextAnimation(new Animation(1062));
			npc.setNextGraphics(new Graphics(252, 0, 100));
			delayHit(npc, 2, target, getMeleeHit(npc, getRandomMaxHit(npc, Utils.random(280), NPCCombatDefinitions.MELEE, target)), getMeleeHit(npc, getRandomMaxHit(npc, Utils.random(280), NPCCombatDefinitions.MELEE, target)));
		}
		return 5;
	}
