...
public class PolyporeCreatureCombat extends CombatScript {
	...
	return new Object[] { 14688, 14689, 14690, 14691, 14692, 14693, 14694, 14695, 14696, 14697, 14698, 14699, 14700,
		14701 };
    }
	...
	if ((npc.getId() == 14700 || npc.getId() == 14701) && Utils.random(5) == 0) {
		...
		for (int dir = 0; dir < dirs[0].length; dir++) {
			...
			npc.setNextForceMovement(new NewForceMovement(new WorldTile(npc), 0, tile, 2,
				Utils.getAngle(tile.getX() - middle.getX(), tile.getY() - middle.getY())));
	...
	if (npc.getAttackStyle() != NPCCombatDefinitions.MELEE) {
		...
		delayHit(npc, 2, target, getRangeHit(npc, getMaxHit(npc, 278, NPCCombatDefinitions.RANGE, target)));
		if (npc.getId() == 14689) {
		...
		delayHit(npc, Utils.projectileTimeToCycles(projectile.getEndTime()) - 1, target,
			getMagicHit(npc, getMaxHit(npc, 320, NPCCombatDefinitions.MAGE, target)));
	    }
	} else
	    delayHit(npc, 0, target, getMeleeHit(npc, getMaxHit(npc, 322, NPCCombatDefinitions.MELEE, target)));
