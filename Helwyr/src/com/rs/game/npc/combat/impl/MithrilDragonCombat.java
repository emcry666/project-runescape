...
public class MithrilDragonCombat extends CombatScript {
	...
	case 3: // melee
	    ...
	    delayHit(npc, 0, target, getMeleeHit(npc, getRandomMaxHit(npc, 325, NPCCombatDefinitions.MELEE, target)));
	    break;
	case 2: // magic
	    npc.setNextAnimation(new Animation(13160));
	    Projectile projectile = World.sendProjectileNew(npc, target, 2705, 28, 16, 35, 2, 16, 0);
	    delayHit(npc, Utils.projectileTimeToCycles(projectile.getEndTime()), target,
		    getMagicHit(npc, getMaxHit(npc, 420, NPCCombatDefinitions.MAGE, target)));
	    break;
	case 1: // range
	    npc.setNextAnimation(new Animation(13160));
	    projectile = World.sendProjectileNew(npc, target, 16, 28, 16, 35, 2, 16, 0);
	    delayHit(npc, Utils.projectileTimeToCycles(projectile.getEndTime()), target,
		    getRangeHit(npc, getMaxHit(npc, 320, NPCCombatDefinitions.RANGE, target)));
	    break;
	case 0: // dragonfire
	    int damage = getMaxHit(npc, 375, NPCCombatDefinitions.MAGE, target);
		...
		boolean hasSuperPot = player.getSuperFireImmune() > Utils.currentTimeMillis();
		boolean hasRegularPot = player.getFireImmune() > Utils.currentTimeMillis();
		...
		if (hasSuperPot)
		    player.getPackets()
			    .sendGameMessage("Your potion fully protects you from the dragon's fiery breath.");
		else if (hasShield)
