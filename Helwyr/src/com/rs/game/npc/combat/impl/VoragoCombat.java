...
public class VoragoCombat extends CombatScript {
			...
			public void run() {
				...
				for (Player p : VoragoHandler.getPlayers()) {
					...
					if (!VoragoHandler.vorago.safePlayers.contains(p)) {
						p.applyHit(new Hit(VoragoHandler.vorago, Utils.random(400, 500), HitLook.REGULAR_DAMAGE));
					}
				}
				if (VoragoHandler.vorago.wfCount == 3) {
	...
	public void sendSmash(NPC npc, Entity target) {
		target.applyHit(new Hit(npc, Utils.random(250, 600), HitLook.REGULAR_DAMAGE));
		...
		if (!Utils.isOnRange(npc, target, 2)) {// If its not in MD then send a
			...
			for (Entity t : VoragoHandler.getPlayers()) {// Hits all players in
				...
				if (Utils.isOnRange(npc, t, 2)) {
					Hit hit = getMeleeHit(npc, getMaxHit(npc, 500, NPCCombatDefinitions.MELEE, t));
	...
	private void sendBlueBomb(final NPC npc, final Boolean anim) {// Blue bombs are
														// sometimes sent
														// without animation
		final Entity farthest = (Entity) getFarthest(npc);
		if (anim) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				final Entity targets[] = VoragoHandler.getPlayers().toArray(new Entity[VoragoHandler.getPlayersCount()]);
				...
				for (Entity t : players) {// Hits all players close to the
					int damage = getMaxHit(npc, 450, NPCCombatDefinitions.MAGE, t);
	...
	private void sendRedBomb(final NPC npc) {
		final Entity farthest = (Entity) getFarthest(npc);
				...
				for (int i = 0; i < targets.length; i++) {
					if (targets[i] != farthest) {
						if (Utils.getDistance(farthest, targets[i]) <= 4) {
							RBDmg = RBDmg + 100;// Adds 1k damage for all
