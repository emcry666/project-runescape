...
public class FareedCombat extends CombatScript {
	...
	public int attack(final NPC npc, Entity target) {
		...
		if (attackStyle == 0) {
			npc.setNextAnimation(new Animation(440));
			...
			World.sendGraphics(npc, new Graphics(453), center);
					...
					for (Player player : World.getPlayers()) { // lets just loop
						...
						if (player.withinDistance(center, 1)) {
							...
							if (count == 1 && player.withinDistance(center, 1)) {
								player.sm("You are badly burnt by Fareed's flames!");
							}
			...
			target.setNextGraphics(new Graphics(310));
			...
			npc.setNextAnimation(new Animation(7070));
