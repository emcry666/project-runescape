...
public class ServantNPC extends NPC {
	...
	String basicResponse = "I appologise, but I cannot serve " + (owner.getAppearence().isMale() ? "Sir" : "Madam")
		+ " without";
	...
	if (kitchen == null) {
	    ...
	    owner.getDialogueManager().startDialogue("SimpleNPCMessage", getId(),
		    basicResponse + " a proper dining room.");
	    return;
	} else {
		...
		if (!kitchen.containsBuild(build)) {
		    owner.getDialogueManager().startDialogue("SimpleNPCMessage", getId(),
			    basicResponse + " a " + build.toString().toLowerCase() + ".");
		    return;
		}
	    }
		...
		public void run() {
			...
			if (count == totalCount + 4)
				...
				for (int y = 0; y < (rotation == 1 || rotation == 3 ? 4 : 2); y++)
				    World.addGroundItem(
					    new Item(builds.length == 6 ? 7736
						    : builds.length == 5 ? house.getServant().getFoodId()
							    : HouseConstants.BEERS[kitchen
								    .getBuildSlot(Builds.BARRELS)]),
					    diningTable.transform(x, y, 0), null, false, 300);
...
//	    owner.getDialogueManager().startDialogue("SimpleNPCMessage", getId(),
//		    "I'm sorry to inform you that I can only hold " + inventorySize + " items during a session.");
//	    return;
//	}
...
//			bank.depositItem(owner.getInventory().getItems().getThisItemSlot(finalItem), completeQuantity,
//				false);
//		    }
//		}
//		owner.getDialogueManager().startDialogue("SimpleNPCMessage", getId(),
//			type == 3
//				? "I have successfully deposited your items into your bank. No longer will the items be at risk from thieves."
