...
public class Vorago extends NPC {
	...
	public int startPushBack = 10000;// This seems to be a decent start point,
	...
	public void bringDownTransform() {
		if (transformCount == 0) {
			...
			int damageRequired = 1500 * VoragoHandler.getPlayersCount();
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
				...
				for (Player t : VoragoHandler.getPlayers()) {
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							new AdventureLog(t, ALogType.BOSS, getName());
						}
	...
	public void sendLandHit() {// TODO make it so it actually hits when vorago
		...
		for (Entity target1 : getPossibleTargets()) {
			if (Utils.colides(Vorago.this, target1)) {
				target1.applyHit(new Hit(Vorago.this, Utils.random(300, 500), HitLook.REGULAR_DAMAGE));
