...
public class DarkEnergyCore extends NPC {
	...
	delay = Utils.projectileTimeToCycles(
		World.sendProjectileNew(this, tile, 1828, 40, 0, 0, 0.7, 20, 0).getEndTime()) - 1;
	...
	if (getId() == 8127) { // not hidden
		...
		setNPCHidden(true);
		delay = Utils.projectileTimeToCycles(
			World.sendProjectileNew(this, tile, 1828, 30, 0, 0, 0.7, 20, 0).getEndTime()) - 1;
		return;
	    }
	...
	beast.heal(damage);
	delay = target.getPoison().isPoisoned() ? 10 : 3;
