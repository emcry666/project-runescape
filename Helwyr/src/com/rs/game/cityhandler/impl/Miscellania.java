...
public class Miscellania implements CityEvent {
	...
	public boolean handleNPCClick(Player player, NPC npc) {
		...
		case 1374:
			player.getDialogueManager().startDialogue(new Dialogue() {
				...
				@Override
				public void start() {
					player(NORMAL, "I probably shouldn't bother this guard.");
				}
	...
	public boolean handleNPCClick2(Player player, NPC npc) {
		...
		case 1397:
		case 3917:
			player.getPackets().sendGameMessage("Your reputation with your kingdom is at " + player.getThrone().getReputation() + "%.");
			return true;
		}
		return false;
	}
	...
	public boolean handleObjectClick(Player player, WorldObject object) {
		...
		case "maple tree":
			player.getActionManager().setAction(new MiscellaniaWoodcutting(object, TreeDefinitions.MAPLE));
			return true;
			
		case "evergreen":
			player.getActionManager().setAction(new MiscellaniaWoodcutting(object, TreeDefinitions.EVERGREEN));
			return true;
			
		case "coal rocks":
			player.getActionManager().setAction(new MiscellaniaMining(object, RockDefinitions.Coal_Ore));
			return true;
