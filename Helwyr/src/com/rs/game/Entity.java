...
import com.rs.game.player.pathing.EntityStrategy;
import com.rs.game.player.pathing.ObjectStrategy;
...
public abstract class Entity extends WorldTile {
	...
	public boolean isBehind(Entity other) {
		if (other.getDirection() <= 2048 || other.getDirection() >= 14336) {
			if (getY() > other.getY()) {
				return true;
			}
		}
		if (other.getDirection() <= 10240 && other.getDirection() >= 6144) {
			if (getY() < other.getY()) {
				return true;
			}
		}
		if (other.getDirection() <= 6144 && other.getDirection() >= 2048) {
			if (getX() > other.getX()) {
				return true;
			}
		}
		if (other.getDirection() <= 14336 && other.getDirection() >= 10240) {
			if (getX() < other.getX()) {
				return true;
			}
		}
	...
	public void removeHitpoints(Hit hit) {
		...
		if (hitpoints <= 0)
			...
			if(pet != null && player.getPetManager().isALegendaryPet() && player.getHitpoints() <= player.getMaxHitpoints() * 0.2) {
				player.heal((int) (player.getMaxHitpoints() * .10));
				Magic.sendNormalTeleportSpell(player, 1, 0, Settings.RESPAWN_PLAYER_LOCATION);
				player.getPackets().sendGameMessage("<col=E86100>Your " + pet.getName() + " saves you, and has given you life points.");
			}
	...
	public abstract void sendInstantDeath(Entity source);
	...
	public final void setLocation(WorldTile tile) {
		...
		if (this instanceof Player) {
			...
			Player player = (Player) this;
			Area nextArea = AreaManager.get(this);
			Area currentArea = player.getCurrentArea();

			if (nextArea == null) {
				player.setCurrentArea(null);
				return;
			}

			if (currentArea == null
					|| !nextArea.name().equals(currentArea.name())) {
				player.setCurrentArea(nextArea);
				nextArea.players().add(player);

				if (player.hasStarted())
					AreaManager.update(player, nextArea);
			}
	...
	public ArrayList<Player> getPlayerDamageSources() {
	...
	public void setNextAnimationNoPriority2(Animation nextAnimation) {
	...
	public boolean isBound() {
		if(getFrozenBlockedDelay() > Utils.currentTimeMillis())
			return true;
		return false;
	}
	...
	public Object getAttribute(String attr) {
		return temporaryAttributes.get(attr);
	}

	public Object removeAttribute(String attr) {
		return temporaryAttributes.remove(attr);
	}

	public void putAttribute(String attr, Object val) {
		temporaryAttributes.put(attr, val);
	}
