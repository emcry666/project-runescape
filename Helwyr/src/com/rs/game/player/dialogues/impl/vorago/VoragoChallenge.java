...
import com.rs.game.npc.vorago.VoragoMisc.VoragoAccChallenge;
				...
				for (Player p : World.getPlayers()) {
					if (p.getX() >= 3029 && p.getX() <= 3055 && p.getY() >= 6117 && p.getY() <= 6136) {
						...
						if (p != player) {
							...
							p.getDialogueManager().startDialogue("VoragoAccChallenge");
						} else {
							...
							for (Player p : players) {
								if (p.getTemporaryAttributtes().get("accRagoChall") != null && ((boolean) p.getTemporaryAttributtes().get("accRagoChall") == true)) {
									...
									if (count == 1) {
										...
										p.applyHit(new Hit(p, (980 / players.size()), HitLook.REGULAR_DAMAGE));
