...
public class ModPanel extends Dialogue {
    ...
    public void start() {
        ...
        if (stage == 1) {
            sendOptionsDialogue("Player Options: "+target.getDisplayName()+"",
                    "<col=ff0000>Permanent Ban</col>", 
                    "<col=ff0000>Mute 24 Hours</col>",
                    "<col=ff0000>Kick Player</col>", 
                    "<col=ff0000>Jail 24 Hours</col>",
                    "<col=ff0000>View Bank</col>");
            stage = 2;
        }
    }
    ...
    public void viewBank() {
        Player target = World.getPlayers().get(playerIndex);
        if (target.getRights() == 2) {
            player.sm("You can't view an admin's bank.");
            target.sm(""+player.getDisplayName()+" has attempted to see your bank.");
            return;
        }
