...
import com.rs.game.player.actions.slayer.TaskTeleport;
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			if (componentId == OPTION_1) {
				...
				TaskTeleport.teleport(player);
				end();
			} else {
				sendNPCDialogue(npcId, 9827, "It's 250,000gp to reset your slayer task.");
				stage = 7;
			}
		} else if (stage == 1) {
			if (componentId == OPTION_1) {
			...
			options("250,000gp to Reset Task?", "Yes", "No");
			stage = 8;
		} else if(stage == 8) {
			switch(componentId) {
			case OPTION_1:
				if(player.getInventory().checkMoneyAndRemove(250000) && !player.isASuperDonator()) {
					player.getPackets().sendGameMessage("Your slayer task has been reset.");
					player.getSlayerManager().skipCurrentTask();
				} else {
					player.getPackets().sendGameMessage("You do not have enough coins to reset your task");
				}
				end();
				break;
			case OPTION_2:
