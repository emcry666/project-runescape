...
public final class CombatDefinitions implements Serializable {
	...
	public boolean isShowCombat() {
		return showCombatSpells;
	}

	public boolean isShowSkill() {
		return showSkillSpells;
	}

	public boolean isShowMisc() {
		return showMiscallaneousSpells;
	}

	public boolean isShowTele() {
		return showTeleportSpells;
	}
	...
	public void setAutoCastSpell2(int id) {
		autoCastSpell = (byte) id;
	}
	...
	public void setSpellBook2(int id) {
		if (id == 3)
			dungeonneringSpellBook = true;
		else
			spellBook = (byte) id;
	...
	public void refreshBonuses() {
		...
		for (final Item item : player.getEquipment().getItems().getItems()) {
			...
			if (bonuses == null)
				continue;
			final int shield = player.getEquipment().getShieldId(), weapon = player.getEquipment().getWeaponId();
			/**
			 * Correctly prevents items like rune knives or the zaryte bow getting ranged bonus from
			 * ammunition, whilst allowing for items such as the noxious longbow or magic shortbow to
			 * acquire bonus. Accounts for crossbows, too.
			 */
			for (int bonus = 0; bonus < bonuses.length; bonus++) {
				if (bonus == RANGED_STR_BONUS) {
					if (item.getDefinitions().getEquipSlot() == Equipment.SLOT_ARROWS) {
						if (this.bonuses[RANGED_STR_BONUS] != 0) {
							if (weapon != -1) {
								final ItemDefinitions wDefs = ItemDefinitions.getItemDefinitions(weapon);
								if (wDefs != null) {
									if (wDefs.isRangedWeapon() 
											&& !wDefs.getName().toLowerCase().contains("cross")
											&& !wDefs.getName().toLowerCase().contains("c'bow")
											&& !wDefs.requiresAmmunitionToFunction())
										continue;
								}
							} else if (shield != -1) {
								final ItemDefinitions sDefs = ItemDefinitions.getItemDefinitions(shield);
								if (sDefs != null) {
									if (sDefs.isRangedWeapon() 
											&& !sDefs.getName().toLowerCase().contains("cross")
											&& !sDefs.getName().toLowerCase().contains("c'bow")
											&& !sDefs.requiresAmmunitionToFunction())
										continue;
								}
							}
						}
				...
				this.bonuses[bonus] += bonuses[bonus];
			}
		}
	}
	...
	public void setSpecialAttack2(int special) {
		specialAttackPercentage = (byte) special;
	}
	...
	public void restoreFullSpecialAttack() {
		if (player.getFamiliar() != null)
			player.getFamiliar().restoreSpecialAttack(60);
		if (specialAttackPercentage == 100)
			return;
		restoreSpecialAttack(10);
		if (specialAttackPercentage == 100 || specialAttackPercentage == 50)
	...
	public void setAttackStyle2(int style) {
	...
	public void setShowSkills(boolean b) {
		this.showSkillSpells = b;
	}

	public void setShowCombat(boolean b) {
		this.showCombatSpells = b;
	}

	public void setShowTele(boolean b) {
		this.showTeleportSpells = b;
	}

	public void setShowMisc(boolean b) {
		this.showMiscallaneousSpells = b;
	}
	

	private boolean isForceNoSheathe() {
		return player.isCanPvp() || FfaZone.inArea(player);
	}
