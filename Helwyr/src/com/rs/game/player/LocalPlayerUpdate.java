...
public final class LocalPlayerUpdate {
	...
	private void applyBodyGlowMask(Player p, OutputStream data) {
		data.writeByte(p.getBodyGlow().getColors()[0]);
		data.writeByteC(p.getBodyGlow().getColors()[1]);
		data.writeByte128(p.getBodyGlow().getColors()[2]);
		data.writeByteC(p.getBodyGlow().getColors()[3]);
		data.writeShort(0);
