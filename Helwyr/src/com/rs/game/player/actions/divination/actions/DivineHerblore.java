...
public class DivineHerblore extends Action {
	...
	DIVINE_HERB_I(87280, 1, -1, new Animation(21237), 10, Herbs.GUAM, Herbs.MARRENTILL, Herbs.TARRONMIN,
		Herbs.HARRALANDER, Herbs.RANARR, Herbs.SPIRIT_WEED, Herbs.TOADFLAX),

	DIVINE_HERB_II(87281, 1, -1, new Animation(21237), 10, Herbs.IRIT, Herbs.WERGALI, Herbs.AVANTOE, Herbs.KWUARM,
		Herbs.SNAPDRAGON),

	DIVINE_HERB_III(87282, 1, -1, new Animation(21237), 10, Herbs.CADANTINE, Herbs.LANTADYME, Herbs.DWARF_WEED,
	...
	private DivineHerbsSpots(int id, int option, int bait, Animation animation, int randomLifeProbability,
		Herbs... herbs) {
	    this.id = id;
	    this.bait = bait;
	...
	int difference = player.getSkills().getLevel(Skills.HERBLORE) - spot.getHerbs()[random].getLevel();
	if (difference < -1)
	    return random = 0;
	if (random < -1)
	    return random = 0;
	...
	Item fish = new Item(spot.getHerbs()[fishId].getId(), multipleCatch ? 2 : 1);
	Item notedfish = new Item(spot.getHerbs()[fishId].getId() + 1, multipleCatch ? 2 : 1);
	...
	if (player == owner && roll >= 60) {
	    ...
	    owner.getPackets().sendGameMessage("" + player.getUsername() + " harvested some " + fishName + " for you.");
	    player.getInventory().addItem(fish);
	...
	if (player.getSkills().getLevel(Skills.HERBLORE) < spot.getHerbs()[fishId].getLevel()) {
	    player.getDialogueManager().startDialogue("SimpleMessage",
		    "You need a herblore level of " + spot.getHerbs()[fishId].getLevel() + " to harvest here.");
	    return false;
	}
	if (!player.getInventory().containsOneItem(spot.getBait()) && spot.getBait() != -1) {
	    player.getPackets().sendGameMessage("You don't have "
		    + new Item(spot.getBait()).getDefinitions().getName().toLowerCase() + " to harvest here.");
	    return false;
	}
