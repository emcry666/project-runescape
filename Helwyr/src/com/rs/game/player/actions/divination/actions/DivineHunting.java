...
public class DivineHunting extends Action {
	...
	DIVINE_KEBBIT_BURROW(87270, 1, -1, new Animation(21236), 10, Herbs.polar_kebbit_fur, Herbs.raw_beast_meat,
		Herbs.common_kebbit_fur, Herbs.feldip_weasel_fur, Herbs.desert_devil_fur, Herbs.long_kebbit_spike),

	DIVINE_BIRD_SNARE(87271, 1, -1, new Animation(21236), 10, Herbs.red_feathers, Herbs.raw_bird_meat,
		Herbs.yellow_feathers, Herbs.orange_feathers, Herbs.blue_feathers, Herbs.stripy_feathers,
		Herbs.wimpy_feathers),

	DIVINE_DEADFALL_TRAP(87272, 1, -1, new Animation(21236), 10, Herbs.kebbit_claws, Herbs.barb_tail_harpoon,
		Herbs.kebbit_spike, Herbs.diseased_kebbit_fur, Herbs.kebbit_teeth),

	DIVINE_BOX_TRAP(87273, 1, -1, new Animation(21236), 10, Herbs.Chinchompas, Herbs.red_chinchompas,
		Herbs.raw_pawya_meat, Herbs.grenwall_spikes);
	...
	private DivineHuntingSpots(int id, int option, int bait, Animation animation, int randomLifeProbability,
		Herbs... herbs) {
	    this.id = id;
	    this.bait = bait;
	...
	int wcTimer = spot.getRandomLifeProbability() - (player.getSkills().getLevel(15) + summoningBonus);
	...
	int difference = player.getSkills().getLevel(Skills.HUNTER) - spot.getHerbs()[random].getLevel();
	if (difference < -1)
	    return random = 0;
	if (random < -1)
	    return random = 0;
	...
	Item hitem = new Item(spot.getHerbs()[fishId].getId(), multipleCatch ? 2 : 1);
	Item hnitem = new Item(spot.getHerbs()[fishId].getId() + 1, multipleCatch ? 2 : 1);
	...
	if (player == owner && roll >= 60) {
	    ...
	    player.getPackets().sendGameMessage("You caught a " + name + ".", true);
	} else {
	    String name = hitem.getDefinitions().getName().toLowerCase();
	    owner.getInventory().addItem(hnitem);
	    owner.getPackets().sendGameMessage("" + player.getUsername() + " caught some " + name + " for you.");
	    player.getInventory().addItem(hitem);
	    player.getPackets().sendGameMessage("You caught some " + name + ".", true);
	}
	...
	if (multipleCatch)
	    return "Your high level in hunter allows you to catch two " + hunt.getDefinitions().getName().toLowerCase()
		    + ".";
	else
	...
	if (player.getSkills().getLevel(Skills.HERBLORE) < spot.getHerbs()[fishId].getLevel()) {
	    player.getDialogueManager().startDialogue("SimpleMessage",
		    "You need a hunter level of " + spot.getHerbs()[fishId].getLevel() + " to hunt here.");
	    return false;
	}
	if (!player.getInventory().containsOneItem(spot.getBait()) && spot.getBait() != -1) {
	    player.getPackets().sendGameMessage("You don't have "
		    + new Item(spot.getBait()).getDefinitions().getName().toLowerCase() + " to hunt here.");
	    return false;
	}
