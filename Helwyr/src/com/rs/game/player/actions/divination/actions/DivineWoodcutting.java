...
public final class DivineWoodcutting extends Action {
	...
	private DivineTreeDefinitions(int level, double xp, int logsId, int logBaseTime, int logRandomTime, int stumpId,
	...
	if (!usedDeplateAurora && (1 + Math.random()) < player.getAuraManager().getChanceNotDepleteMN_WC()) {
		...
		&& definitions != DivineTreeDefinitions.DIVINE_MAGIC && definitions != DivineTreeDefinitions.DIVINE_YEW
		&& definitions != DivineTreeDefinitions.DIVINE_MAPLE
		&& definitions != DivineTreeDefinitions.DIVINE_WILLOW && definitions != DivineTreeDefinitions.DIVINE_OAK
	...
	if (player == owner && roll >= 60) {
	    ...
	    owner.getPackets().sendGameMessage("" + player.getUsername() + " cut some " + logName + " for you.");
	    player.getInventory().addItem(log, 1);
