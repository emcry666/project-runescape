...
public class DivineFishing extends Action {
	...
	private DivineFishingSpots(int id, int option, int bait, Animation animation, Fish... fish) {
	    this.id = id;
	    this.bait = bait;
	...
	Item notedfish = new Item(spot.getFish()[fishId].getId() + 1, multipleCatch ? 2 : 1);
	...
	if (bigFish == 1) {
		...
		owner.getPackets()
			.sendGameMessage("" + player.getDisplayName() + " caught some " + fishName + " for you.");
		player.getInventory().addItem(fish);
