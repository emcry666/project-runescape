...
public class SlayerManager implements Serializable {
	...
	public SlayerTask[] canceledTasks = new SlayerTask[6];
	...
	protected int completedTasks, slayerPoints, maximumTaskCount,
			currentTaskCount;
	public boolean[] learnedAbilities = new boolean[7];
	...
	public boolean isValidTask(String name, int id) {
		if (currentTask == null)
			return false;
		List<SlayerTask> tasks = new LinkedList<SlayerTask>(
		...
		for (SlayerTask currentTask : tasks) {
			...
			if (currentTask.toString().replace("_", " ").toLowerCase()
					.equals("monkey")
					&& name.contains("monkey")) {
				return true;
			}
			if (currentTask == SlayerTask.TZHAAR && id == 2745) {
				maximumTaskCount = maximumTaskCount - 1;
				player.getSkills().addXp(Skills.SLAYER, 8000);
				return true;
			}
		}
		return false;
	...
	public void setCurrentTaskAmount(int int1) {
		this.currentTaskCount = int1;

	}

	public void setCompletedTasks(int int1) {
		this.completedTasks = int1;

	}

	public void setLearnedAbility(int i, boolean value) {
		this.learnedAbilities[i] = value;

	}

	public void setCanceledTask(int i, String string) {
		SlayerTask task = SlayerTask.getTaskByName(string);
		if (task != null) {
			this.canceledTasks[i] = task;
		}
