...
public class Farming implements Serializable {
	...
	public static enum Herbs {
		GUAM(5291, 199, 9, 15, 17, 4), MARRENTILL(5292, 201, 14, 20, 22, 11), TARROMIN(5293, 203, 19, 24, 26, 18), HARRALANDER(5294, 205, 26, 30, 32, 25), RANARR(5295, 207, 32, 35, 37, 32), TOADFLAX(5296, 3049, 38, 40, 42, 39), IRIT(5297, 209, 44, 48, 50, 46), AVANTOE(5298, 211, 50, 60, 62, 53), WERGALI(14870, 14836, 46, 54, 56, 60), KWUARM(5299, 213, 56, 69, 78, 69), SNAPDRAGON(5300, 3051, 62, 87.5, 98.5, 75), CADANTINE(5301, 215, 67, 106.5, 120, 82), LANTADYME(5302, 2485, 73, 134.5, 151.5, 89), DWARF_WEED(5303, 217, 79, 170.5, 192, 96), TORSTOL(5304, 219, 85, 199.5, 22.5, 103), FELLSTALK(21621, 21626, 91, 225, 315.6, 103);
	...
	public static enum Allotments {
		POTATO(5318, 1942, 1, 8, 10, 6, 4), ONION(5319, 1957, 5, 12, 14, 13, 4), CABBAGE(5324, 1965, 7, 14, 16, 20, 4), TOMATOES(5322, 1982, 12, 21, 23, 27, 4), SWEETCORN(5320, 5986, 20, 29, 31, 34, 6), STRAWERRIES(5323, 5504, 31, 39, 41, 43, 6), WATERMELON(5321, 5982, 47, 48.5, 54.5, 52, 8);
	...
	public static enum Flowers {
		MARIGOLD(5096, 6010, 2, 8.5, 15, 8, 4), ROSEMARY(5097, 6014, 11, 12, 23, 13, 4), 
		NASTURTIUM(5098, 6012, 24, 19.5, 25, 18, 4), WOAD(5099, 1793, 25, 20.5, 29, 23, 4), 
		LIMPWURT(5100, 225, 26, 21.5, 35, 28, 5), LILY(14589, 14583, 52, 70, 74.8, 37, 4);
	...
	public static void startRake(final Player player, final int objectId) {
		if (player.getInventory().getFreeSlots() < 3) {
			...
			player.sm("You need a rake to rake these weeds.");
			return;
		}
	}
	...
	public static void plantTreeSapling(final Player player, final int sapling, final int objectId) {
		...
		if (player.getSkills().getLevelForXp(Skills.FARMING) < tree.getReq()) {
			player.sm("You need a Farming level of " + tree.getReq() + " to plant this.");
			return;
		}
		if (player.getInventory().containsItem(SEED_DIBBER, 1)) {
			if (Trees.trees.containsKey(sapling)) {
				...
				player.sm("You can't plant that here.");
				return;
			}
		} else {
			player.sm("You need a seed dibber to plant this sapling.");
			return;
		}
	}
	...
	public static void plantHerbSeed(final Player player, final int seed, final int objectId) {
		...
		if (player.getSkills().getLevelForXp(Skills.FARMING) < herb.getReq()) {
			player.sm("You need a Farming level of " + herb.getReq() + " to plant this.");
			return;
		}
		if (player.getInventory().containsItem(SEED_DIBBER, 1)) {
			...
			player.sm("You need a seed dibber to plant these seeds.");
			return;
		}
	}
	...
	public static void plantAllotmentSeed(final Player player, final int seed, final int objectId) {
		...
		if (player.getSkills().getLevelForXp(Skills.FARMING) < allotment.getReq()) {
			player.sm("You need a Farming level of " + allotment.getReq() + " to plant this.");
			return;
		}
		if (player.getInventory().containsItem(SEED_DIBBER, 1) && player.getInventory().containsItem(allotment.getSeed(), 3)) {
			...
			player.sm("You need atleast three seeds to plant.");
			return;
		}
	}
	...
	public static void plantFlowerSeed(final Player player, final int seed, final int objectId) {
		...
		if (player.getSkills().getLevelForXp(Skills.FARMING) < flower.getReq()) {
			player.sm("You need a Farming level of " + flower.getReq() + " to plant this.");
			return;
		}
		if (player.getInventory().containsItem(SEED_DIBBER, 1)) {
	...
	public static void harvestTreeSapling(final Player player, final int objectId, int sapling) {
		...
		if (player.getInventory().getFreeSlots() < 3) {
			player.sm("You need atleast three free inventory spaces to harvest.");
		} else if (weaponId != -1) {
			switch (weaponId) {
			...
			player.sm("You need a hatchet to chop down this tree.");
			return;
		}
	}
	...
	public static void harvestHerb(final Player player, final int objectId, int seed) {
		...
		if (player.getInventory().getFreeSlots() < 3) {
			...
			player.sm("You need secateurs to harvest these herbs.");
			return;
		}
	}
	...
	public static void harvestAllotment(final Player player, final int objectId, int seed) {
		...
		if (player.getInventory().getFreeSlots() < 3) {
			...
			player.sm("You need a spade to harvest this allotment.");
			return;
		}
	}
	...
	public static void harvestFlower(final Player player, final int objectId, int seed) {
		...
		if (player.getInventory().getFreeSlots() < 3) {
			...
			player.sm("You need a spade to harvest this flower.");
			return;
		}
	}
