...
import com.rs.game.player.actions.farming.PatchConstants.AllotmentPatch;
import com.rs.game.player.actions.farming.PatchConstants.BellPatch;
import com.rs.game.player.actions.farming.PatchConstants.BushPatch;
import com.rs.game.player.actions.farming.PatchConstants.CactusPatch;
import com.rs.game.player.actions.farming.PatchConstants.FlowerPatch;
import com.rs.game.player.actions.farming.PatchConstants.FruitTreePatch;
import com.rs.game.player.actions.farming.PatchConstants.HerbPatch;
import com.rs.game.player.actions.farming.PatchConstants.HopPatch;
import com.rs.game.player.actions.farming.PatchConstants.MushroomPatch;
import com.rs.game.player.actions.farming.PatchConstants.TreePatch;
import com.rs.game.player.actions.farming.PatchConstants.VineHerbPatch;
...
public class Patch implements Serializable {
	...
	public void handlePlanting(Player player, int seedId) {
		if (!player.getInventory().containsItemToolBelt(PatchConstants.SEED_DIBBER, 1)) {
	...
	public void handleOption1(Player player) {
		if (!raked) {
			if (!player.getInventory().containsItemToolBelt(PatchConstants.RAKE, 1)) {
		...
		if (grown) {
			if (FruitTreePatch.forId(currentSeed) != null) {
				...
				if (!healthChecked) {
					...
					if (yield == 0) {
						if (player.getInventory().containsItemToolBelt(PatchConstants.SPADE, 1)) {
