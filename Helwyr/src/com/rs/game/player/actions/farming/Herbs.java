...
public class Herbs {
	...
	public static void CheckGuam(Player player) {
		if (!player.getInventory().containsItem(DIBBLER, 1)) {
			player.sm("You need a seed dibbler to plant this.");
		} else if (!player.getInventory().containsItem(GUAM, 1)) {
			player.sm("You need a guam seed to plant.");
		} else {
			...
			player.sm("You successfully plant the Guam.");
		}
	}
	...
	public static void CheckSnapdragon(Player player) {
		if (!player.getInventory().containsItem(DIBBLER, 1)) {
			...
			player.sm("You need a snapdragon seed to plant.");
		} else {
			...
			player.setNextAnimation(USEDIBBLER);
			player.sm("You successfully plant the Spandragon.");
		}
	}
	...
	public static void CheckDwarfweed(Player player) {
		if (!player.getInventory().containsItem(DIBBLER, 1)) {
			...
			player.sm("You need a dwarf weed seed to plant.");
		} else {
			...
			player.setNextAnimation(USEDIBBLER);
			player.sm("You successfully plant the Dwarf Weed.");
		}
	}
}