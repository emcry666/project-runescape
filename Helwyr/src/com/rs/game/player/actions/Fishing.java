...
public class Fishing extends Action {
	...
	public enum Fish {
		...
		BLUE_CRAB(17813, 85, 110),
		
		SALVE_EEL(17811, 90, 180);
	...
	public enum FishingSpots {
		...
		NET2(952, 1, 303, -1, new Animation(621), Fish.SHRIMP), HARPOON3(3848, 2, 311, -1, new Animation(618),
			Fish.TUNA, Fish.SWORDFISH), NET3(3848, 1, 303, -1, new Animation(621), Fish.MONKFISH);
	...
	private void addFish(Player player) {
		...
		if (hasFishingSuit(player))
			totalXp *= 1.025;
		if(spot.getFish()[fishId] == Fish.ROCKTAIL) {
			AchievementLogger.handleFishing(player);
		}
	...
	private boolean checkAll(Player player) {
		...
		if (!player.getInventory().containsOneItem(spot.getTool()) 
				&& !player.getToolbelt().containsItem(spot.getTool())) {
			player.getPackets()
					.sendGameMessage(
							"You need a "
