...
import com.rs.game.player.controlers.CitadelControler;
...
public class Mining extends MiningBase {
	...
	public static enum RockDefinitions {
		...
		Iron_Ore(15, 70, 440, 15, 1, 11552, 10, 0), 
		
		Sandstone_Ore(35, 30, 6971, 30, 1, 11552, 10, 0), 
		
		Silver_Ore(20, 80, 442, 25, 1, 11552, 20, 0), 
		
		Coal_Ore(30, 100, 453, 50, 10, 11552, 30, 0), 
	
		Granite_Ore(45, 50, 6979, 50, 10, 11552, 20, 0), 
		
		Gold_Ore(40, 110, 444, 80, 20, 11554, 40, 0), 
		
		Mithril_Ore(55, 160, 447, 100, 20, 11552, 60, 0), 
		
		Adamant_Ore(70, 180, 449, 130, 25, 11552, 180, 0), 
		
		Runite_Ore(85, 200, 451, 150, 30, 11552, 360, 0), 
		
		LRC_Coal_Ore(77, 110, 453, 50, 10, -1, -1, -1), 
		
		LRC_Gold_Ore(80, 130, 444, 40, 10, -1, -1, -1),
	...
	private int getMiningDelay(Player player) {
		...
		if(definitions == RockDefinitions.STONE_CLIPPINGS) {
			 mineTimer = 12;
		} else {
		...
		if (definitions.getEmptyId() != -1) {
			if (!usedDeplateAurora && (1 + Math.random()) < player.getAuraManager().getChanceNotDepleteMN_WC()) {
				...
				if (rock.spawnedByEd) {
					...
					World.spawnTemporaryObject(new WorldObject(definitions.getEmptyId(), rock.getType(), rock.getRotation(), rock.getX(), rock.getY(), rock.getPlane()), (player.isAExtremeDonator() ? definitions.respawnDelay / 2 : definitions.respawnDelay) * 600, false);
				}
	...
	private void addOre(Player player) {
		...
		if(definitions == RockDefinitions.Gold_Ore ||
				definitions == RockDefinitions.LRC_Gold_Ore) {
			AchievementLogger.handleMining(player);
		}
