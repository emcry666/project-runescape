...
public abstract class MiningBase extends Action {
	...
	protected boolean hasPickaxe(Player player) {
		...
		if (player.getInventory().containsOneItem(15259, 1275, 1271, 1273, 1269, 1267, 1265, 13661, 32646))
			return true;
	...
	public static PickAxeDefinitions getPickAxeDefinitions(Player player, boolean dungeoneering) {
		for (int i = dungeoneering ? 10 : PickAxeDefinitions.values().length - 1; i >= (dungeoneering ? 0 : 11); i--) { // from
			...
			if (player.getInventory().containsItem(def.pickAxeId)
