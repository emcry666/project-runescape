...
public class PickPocketAction extends Action {
	...
	public int processWithDelay(Player player) {
		if (!isSuccesfull(player)) {
			...
			AchievementLogger.handleThieving(player);
			if (player.getDailyTask() != null)
				player.getDailyTask().incrementTask(player, 1, npc.getId(), Skills.THIEVING);
			if (npcData == PickPocketableNPC.IORWERTH_WORKERS)
				player.getInventory().addItem(generateIorwerthLoot());

			for (int i = 0; i <= index; i++) {
				Item item = npcData.getLoot()[Utils.random(npcData.getLoot().length)];
				player.getPackets().sendRunScript(5561, 1, item.getAmount());
				player.getMoneyPouch().addToMoneyPouch(item.getAmount());
			}
		}
		return -1;
	}
	
	public static Item generateIorwerthLoot() {
		Item item = null;
		if (Utils.random(500) == 1) {
			Item[] rare = { new Item(32206, 1), new Item(13845, 1), new Item(32736, 1), new Item(32732, 1), new Item(32733, 1), new Item(32734, 1), new Item(32735, 1), new Item(31427, 1), new Item(13010, 1), new Item(19064, 1) };
			item = rare[Utils.random(0, rare.length - 1)];
		} else {
			Item[] common = { new Item(995, Utils.random(4135, 8746)), new Item(450, 1), new Item(168, 1), new Item(150, 1), new Item(162, 1), new Item(385, 1) };
			item = common[Utils.random(0, common.length - 1)];
		}
		return item;
	}
