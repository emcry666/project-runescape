...
public class Fletching extends Action {
	...
	public static final Item DUNGEONEERING_KNIFE = null;
	...
	public static enum Fletch {
		...
		STRUNG_MAGIC_LONG_BOW(70, 1777, new int[] { 859 }, new int[] { 85 },
				...
				BLOODWOOD(24121, 946, new int[] { 24122 }, new int[] { 93 },
						new double[] { 13 }, new Animation(6702)),
		...
		IRON_BOLT(9377, 314, new int[] { 9140 }, new int[] { 39 },
				new double[] { 1.5 }, new Animation(-1)),

		STEEL_BOLT(9378, 314, new int[] { 9141 }, new int[] { 46 },
		...
		EMERALD_BOLTS(9190, 9142, new int[] { 9338 }, new int[] { 59 },
		...
		BOLAS(21359, 21358, new int[] { 21365 }, new int[] { 87 },
				new double[] { 50.0 }, new Animation(3114));
	...
	public boolean start(Player player) {
		...
		if (player.getSkills().getLevel(Skills.FLETCHING) < fletch.getLevel()[option]) {
			player.getPackets().sendGameMessage("You need a fletching level of "+fletch.getLevel()[option]+" to fletch this.");
			return false;
		}
		if (!process(player))
			return false;
	...
	public boolean process(Player player) {
		if (ticks <= 0)
			return false;
		if (!player.getInventory().containsItemToolBelt(fletch.getId(), 1)
				|| !player.getInventory().containsItemToolBelt(fletch.getSelected(), 1))
			return false;
			if ((fletch.getId() == 13279 || fletch.getId() == 13278)) {
	 ...
	 public static boolean isFletching(Player player, int logId) {
	       for (Fletch fletch : Fletch.values()) {
	           if (!player.getInventory().containsItemToolBelt(KNIFE, 1)) {
	...
	public static boolean maxMakeQuantityTen(Fletch fletch) {
		return fletch.getSelected() == 52 
				|| ItemDefinitions.getItemDefinitions(fletch.getSelected()).getName().toLowerCase().contains("dart")
				|| ItemDefinitions.getItemDefinitions(fletch.getSelected()).getName().toLowerCase().contains("bolt")
				|| ItemDefinitions.getItemDefinitions(fletch.getSelected()).getName().toLowerCase().contains("arrow")
				|| fletch.getSelected() == 53 
				|| fletch.getSelected() == 24122
				|| fletch.getSelected() == 24121
				|| fletch.getSelected() == 314; //|| ItemDefinitions.getItemDefinitions(fletch.getSelected()).getName().toLowerCase().contains("dart");
	}
	...
	public int processWithDelay(Player player) {
		...
		int amount2 = player.getInventory().getNumerOf(fletch.getSelected());//fletch.getId());
		int amount3 = player.getInventory().getNumerOf(fletch.getId());
		
		if (amount >= amount2 && amount3 >= amount2) {
			player.setNextAnimation(fletch.getAnim());
			player.getInventory().deleteItem(fletch.getId(), fletch.getProduct()[option] == 52 ? 1 : amount2);
			...
			player.getInventory().deleteItem(fletch.getSelected(), amount2);
			}
			...
			player.getInventory().addItem(fletch.getProduct()[option], (fletch.getProduct()[option] == 52 || fletch.getProduct()[option] == 24122) ? 15 : amount2);
			...
			player.getSkills().addXp(Skills.FLETCHING, fletch.getXp()[option] * (amount2 < 1 ? 1 : amount2));
			return 1;
		}
		if (amount >= amount3 && amount2 >= amount3) {
			player.setNextAnimation(fletch.getAnim());
			player.getInventory().deleteItem(fletch.getId(), fletch.getProduct()[option] == 52 ? 1 : amount2);
			if (player.getDailyTask() != null && ItemDefinitions.getItemDefinitions(fletch.getId()).getName().toLowerCase().contains("(u)"))
				//handles stringing unstrung bows
				player.getDailyTask().incrementTask(player, 3, fletch.getId(), Skills.FLETCHING);
			if (fletch.getSelected() != KNIFE) {
			player.getInventory().deleteItem(fletch.getSelected(), amount3);
			}
			player.getPackets().sendGameMessage("You attempt to create a "+ new Item(fletch.getProduct()[option]).getDefinitions().getName().replace("(u)", "."),true);
			player.getInventory().addItem(fletch.getProduct()[option], fletch.getProduct()[option] == 52 ? 15 : amount3);
			player.getPackets().sendGameMessage("You successfully create a "+ new Item(fletch.getProduct()[option]).getDefinitions().getName().replace("(u)", "."), true);
			if (player.getDailyTask() != null 
					&& (ItemDefinitions.getItemDefinitions(fletch.getProduct()[option]).getName().toLowerCase().contains("(u)") || 
							ItemDefinitions.getItemDefinitions(fletch.getProduct()[option]).getName().toLowerCase().contains("bolas")))
						//handles producting unfinished bows and bolas
						player.getDailyTask().incrementTask(player, 3, fletch.getProduct()[option], Skills.FLETCHING);
			player.getSkills().addXp(Skills.FLETCHING, fletch.getXp()[option] * (amount3 < 1 ? 1 : amount3));
			return 1;
		}
		if (amount2 >= amount && amount3 >= amount) {
			player.setNextAnimation(fletch.getAnim());
			...
			player.getPackets().sendGameMessage("You attempt to create a "+ new Item(fletch.getProduct()[option]).getDefinitions().getName().replace("(u)", "."),true);
			player.getInventory().addItem(fletch.getProduct()[option], fletch.getProduct()[option] == 52 ? 15 : amount);
			player.getPackets().sendGameMessage("You successfully create a "+ new Item(fletch.getProduct()[option]).getDefinitions().getName().replace("(u)", "."), true);
			if (player.getDailyTask() != null 
			&& (ItemDefinitions.getItemDefinitions(fletch.getProduct()[option]).getName().toLowerCase().contains("(u)") || 
					ItemDefinitions.getItemDefinitions(fletch.getProduct()[option]).getName().toLowerCase().contains("bolas")))
				//handles producting unfinished bows and bolas
				player.getDailyTask().incrementTask(player, 3, fletch.getProduct()[option], Skills.FLETCHING);
			player.getSkills().addXp(Skills.FLETCHING, fletch.getXp()[option] * (amount < 1 ? 1 : amount));
			return 1; 
		}
		if (fletch.getSelected() != KNIFE && fletch.getSelected() != CHISEL) {
			if (amount2 >= amount && amount3 >= amount2) {
			player.getInventory().deleteItem(fletch.getSelected(), amount2);
			player.getPackets().sendGameMessage("You attempt to create a "+ new Item(fletch.getProduct()[option]).getDefinitions().getName().replace("(u)", "."),true);
			player.getInventory().addItem(fletch.getProduct()[option], amount2);
			player.getPackets().sendGameMessage("You successfully create a "+ new Item(fletch.getProduct()[option]).getDefinitions().getName().replace("(u)", "."), true);
			if (player.getDailyTask() != null 
					&& (ItemDefinitions.getItemDefinitions(fletch.getProduct()[option]).getName().toLowerCase().contains("(u)") || 
							ItemDefinitions.getItemDefinitions(fletch.getProduct()[option]).getName().toLowerCase().contains("bolas")))
		...
		if (fletch.getSelected() != KNIFE && fletch.getSelected() != CHISEL) {
			if (amount3 >= amount && amount2 >= amount3) {
			player.getInventory().deleteItem(fletch.getSelected(), amount3);
			player.getPackets().sendGameMessage("You attempt to create a "+ new Item(fletch.getProduct()[option]).getDefinitions().getName().replace("(u)", "."),true);
