...
public class Herblore extends Action {
	...
	public enum Ingredients {
		...
		ERZILLE(19989, new short[] { Herblore.JUJU_VIAL }, new short[] { 19998 }, new byte[] { 0 }, new double[] { 0 }),

		ARGWAY(19990, new short[] { Herblore.JUJU_VIAL }, new short[] { 20000 }, new byte[] { 0 }, new double[] { 0 }),

		UGUNE(19991, new short[] { Herblore.JUJU_VIAL }, new short[] { 19999 }, new byte[] { 0 }, new double[] { 0 }),

		SHENGO(19992, new short[] { Herblore.JUJU_VIAL }, new short[] { 20001 }, new byte[] { 0 }, new double[] { 0 }),

		SAMADEN(19993, new short[] { Herblore.JUJU_VIAL }, new short[] { 20002 }, new byte[] { 0 }, new double[] { 0 }),

		CORRUPT_VINE(19979, new short[] { 19998 }, new short[] { 20023 }, new byte[] { 54 }, new double[] { 123 }),

		SHADOW_VINE(19977, new short[] { 20000 }, new short[] { 20027 }, new byte[] { 59 }, new double[] { 135 }),

		MARBLE_VINE(19980, new short[] { 19999 }, new short[] { 20011 }, new byte[] { 64 }, new double[] { 146 }),

		PLANT_TEETH(19975, new short[] { 20001 }, new short[] { 20007 }, new byte[] { 67 }, new double[] { 152 }),

		AQUATIC_VINE(19976, new short[] { 20001 }, new short[] { 20019 }, new byte[] { 70 }, new double[] { 158 }),

		OILY_VINE(19972, new short[] { 20002 }, new short[] { 20015 }, new byte[] { 71 }, new double[] { 160 }),

		DRACONIC_VINE(19973, new short[] { 20002 }, new short[] { 20003 }, new byte[] { 74 }, new double[] { 168 }),

		SARADOMIN_VINE(19981, new short[] { 20002 }, new short[] { 20031 }, new byte[] { 75 }, new double[] { 170 }),

		GUTHIX_VINE(19982, new short[] { 20002 }, new short[] { 20035 }, new byte[] { 75 }, new double[] { 170 }),

		ZAMORAK_VINE(19983, new short[] { 20002 }, new short[] { 20039 }, new byte[] { 75 }, new double[] { 170 }),
		...
		MORCHELLA_MUSHROOM(21622, new short[] { 21628 }, new short[] { 21630 }, new byte[] { 94 }, new double[] { 190 }),
	...
	public static final short JUJU_VIAL = 19994;
	...
	public boolean process(Player player) {
		if (ingredients == Ingredients.TORSTOL && otherItem.getId() != VIAL) {
			if (!player.getInventory().containsOneItem(15309) || !player.getInventory().containsOneItem(15313) || !player.getInventory().containsOneItem(15317) || !player.getInventory().containsOneItem(15321) || !player.getInventory().containsOneItem(15325)) {
				player.sm("You don't have the ingredients for an overload.");
				return false;
			}
		}
		if (!player.getInventory().containsItem(node.getId(), 1) || !player.getInventory().containsItem(otherItem.getId(), 1)) {
			player.sm("You ran out of ingredients.");
			return false;
		}
		return true;
	}
	...
	public int processWithDelay(Player player) {
		...
		ticks--;
		if ((node.getId() == VIAL && otherItem.getId() == COCONUT_MILK) || (otherItem.getId() == VIAL && node.getId() == COCONUT_MILK)) {
			player.sm("You add the " + node.getDefinitions().getName().toLowerCase().replace("clean", "") + " into the vial of " + (otherItem.getId() == VIAL ? "water." : "milk."));
		} else if (otherItem.getId() == SWAMP_TAR || node.getId() == SWAMP_TAR) {
			player.sm("You add the " + node.getDefinitions().getName().toLowerCase().replace("clean ", "") + " on the swamp tar.");
		} else if (otherItem.getId() == PESTLE_AND_MORTAR || node.getId() == PESTLE_AND_MORTAR) {
			player.sm("You crush the " + node.getDefinitions().getName().toLowerCase() + " with your pestle and mortar.");
		} else if (ingredients == Ingredients.TORSTOL && otherItem.getId() != VIAL) {
			player.sm("You combine the torstol with the potions and get an overload.");
			...
			player.sm("You mix the " + node.getDefinitions().getName().toLowerCase() + " into your potion.");
		}
		if (!(ingredients == Ingredients.TORSTOL && otherItem.getId() != VIAL)) {
			player.getInventory().deleteItem(new Item(node.getId(), 1));
			player.getInventory().deleteItem(rawIngredient == null ? new Item(otherItem.getId(), 1) : null);
		}
		player.getInventory().addItemMoneyPouch(rawIngredient != null ? rawIngredient.getCrushedItem() : new Item(ingredients.getRewards()[slot], 1));
	...
	public boolean start(Player player) {
		if (player == null || node == null) {
			player.sm("Either you or the potion you are making is null.");
			return false;
		}
		if ((ingredients == null && rawIngredient == null) || otherItem == null) {
			player.sm("That potion doesn't exist yet.");
			return false;
		}
		if (ingredients != null) {
