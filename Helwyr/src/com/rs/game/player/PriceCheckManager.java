...
public class PriceCheckManager {
	...
	public void refreshItems(Item[] itemsBefore) {
		...
		for (int index = 0; index < itemsBefore.length; index++) {
			...
			if (item != null)
				totalPrice += ItemDefinitions.getItemDefinitions(item.getId()).getValue(item.getId())
						* item.getAmount();
			if (itemsBefore[index] != item) {
				...
				player.getPackets()
						.sendGlobalConfig(
								700 + index,
								item == null ? 0 : ItemDefinitions.getItemDefinitions(item.getId()).getValue(item.getId()));
			}
