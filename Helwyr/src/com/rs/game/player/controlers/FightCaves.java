...
public class FightCaves extends Controler {
			...
			if (type == 1 || type == 4) {
				...
				if (type == 4) {
					...
					if (!player.getInventory().hasFreeSlots()) {
						...
						player.getPackets().sendGameMessage(
								"A fire cape has been added to your bank.");
					} else {
						player.getInventory().addItem(6570, 1);
					}
					if (!player.getInventory().hasFreeSlots()) {
						// World.addGroundItem(new Item(6529, 16064), new
						// WorldTile(player), player, true, 180,true);
						player.getBank().addItem(6529, 16064, true);
						player.getPackets().sendGameMessage(
								"16064x Tokkul has been added to your bank.");
					} else {
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		player.getDialogueManager().startDialogue("SimpleMessage",
				"You cannot teleport out of the fight caves!");
		return false;
	}
