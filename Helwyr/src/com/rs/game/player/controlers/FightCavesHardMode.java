...
import com.rs.game.npc.fightcaves.FightCavesHardModeNPC;
import com.rs.game.npc.fightcaves.TzKekCavesHardMode;
import com.rs.game.npc.fightcaves.TzTok_JadHard;
...
public class FightCavesHardMode extends Controler {
	...
	private final int[][] WAVES = {
			...
			,{2741, 2741}
			,{2741, 2741, 2739}
			,{2741, 2741, 2739, 2739, 2734}
			,{2741, 2741, 2741, 2739, 2739}
			,{2741, 2741, 2741, 2741}
			,{2743}
			,{2743, 2741, 2739, 2736, 2734}
			,{2743, 2741, 2741, 2739, 2739}
			,{2743, 2743, 2741}
			,{2743, 2743, 2741, 2741}
			,{2743, 2743, 2743}
			,{2743, 2743, 2743, 2739, 2739, 2741}
			,{2743, 2743, 2743, 2741, 2741}
			,{2743, 2743, 2743, 2743}
			,{2745}
			,{2745, 2745}
	...
	public static void enterFightCavesHardMode(Player player) {
		...
		player.getControlerManager().startControler("FightCavesHardModeControler", 1); //start at wave 1
	}
	...
	public WorldTile getSpawnTile() {
		...
		case 5:
			return getWorldTile(11, 55);
		case 6:
			default:
			return getWorldTile(30, 32);
		}
	}
	...
	public void startWave() {
		...
		for(int id : WAVES[currentWave-1]) {
			if(id == 2736)
				new TzKekCavesHardMode(id, getSpawnTile());
			else if (id == 2745)
				new TzTok_JadHard(id, getSpawnTile(), this);
			else
				new FightCavesHardModeNPC(id, getSpawnTile());
		}
		spawned = true;
	}
	...
	public void spawnHealers() {
		if(stage != Stages.RUNNING)
			return;
		for(int i = 0; i < 4; i++)
			new FightCavesHardModeNPC(2746, getSpawnTile());
	}
	...
	public void setWaveEvent() {
		if(getCurrentWave() == 22) 
			player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "HAHAHA, You thought it was over? Now comes the real challenge!");
		GameEngine.fastExecutor.schedule(new TimerTask() {
	...
	public void exitCave(int type) {
		...
		if(type == 0 || type == 2)
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					player.setCompletedFightCavesHardMode();
					player.reset();
					player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "I am most impressed, you have earned the TzHaar's respect. Please accept this.");
					...
					if(!player.getInventory().addItem(6529, 16064)) 
							World.addGroundItem(new Item(6529, 16064), new WorldTile(player), player, true, 180,true);
					if(!player.getInventory().addItem(24359, 1)) 
							World.addGroundItem(new Item(24359, 1), new WorldTile(player), player, true, 180,true);
					if(!player.getInventory().addItem(24360, 1)) 
							World.addGroundItem(new Item(24360, 1), new WorldTile(player), player, true, 180,true);
					if(!player.getInventory().addItem(24363, 1)) 
							World.addGroundItem(new Item(24363, 1), new WorldTile(player), player, true, 180,true);
					}
					...
					tokkul *= Settings.getDropRate(); //10x more
