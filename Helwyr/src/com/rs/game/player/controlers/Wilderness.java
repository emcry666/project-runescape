...
public class Wilderness extends Controler {
	...
	@Override
	public void start() {
		System.out.println("Plyer:" + player.getUsername() + " IS STARTING CONTROLLER.");
		checkBoosts(player);
	}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					//if (player.getCurrentArea() instanceof BountyHunterArea)
					//	BountyHunter.get().killed(killer, player);
					if(player instanceof Bot)
						player.realFinish();
					player.getEquipment().init();
					player.getInventory().init();
					player.reset();
					player.setNextWorldTile(new WorldTile(3093, 3496, 0));
