...
public class DungeonController extends Controler {
	...
	public void showDeaths() {
		player.getInterfaceManager().setOverlay(945, true);
	}
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		...
		if (dungeon.isAtRewardsScreen()) {
			...
			if (interfaceId == 548) {
				...
				return true;
			} else if (interfaceId == Summoning.INTERFACE) {
	...
	public boolean processItemOnPlayer(Player p2, Item item, int slot) {
		if (Foods.isConsumable(item))
			Foods.eat(p2, item, slot);
		return true;
	}
