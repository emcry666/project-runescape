...
public class HouseControler extends Controler {
	...
	if (npc instanceof ServantNPC) {
	    ...
	    if (!house.isOwner(player)) {
		player.getDialogueManager().startDialogue("SimpleNPCMessage", npc.getId(),
			"The servant ignores your request.");
		return false;
	    }
	...
	if (npc instanceof ServantNPC) {
	    ...
	    player.getDialogueManager().startDialogue("ItemOnServantD", npc, item.getId(),
		    house.getServant().isSawmill());
	    return false;
	}
	return false;
    }
	...
	if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId())
	    ...
	    //ItemTransportation.transportationDialogue(player, new Item(1712, 1), false); TODO
	    return false;
		...
		player.getPackets().sendGameMessage("You cannot do this while in building mode.");
		return false;
	    } else if (!player.getInventory().containsItem(590)
		...
		player.useStairs(4853, new WorldTile(player.getX() == object.getX() ? object.getX() - 1 : object.getX(),
			object.getY(), object.getPlane()), 0, 2);
		...
		player.getPackets()
			.sendGameMessage("Your house must be in build mode in order to change your portal's focus.");
		return false;
	    }
	...
	if (ringType == 1 || ringType == 2) {
		...
		if (item != null) {
		    player.getPackets().sendGameMessage(
			    "You need to remove your " + item.getName() + " before entering the ring.");
		    return false;
		}
	    }
	    if (ringType == 1) {
	...
	if (ringType == 1 || ringType == 2) {
	    boolean distanceAttack = player.getCombatDefinitions().getAttackStyle() == CombatDefinitions.MAGIC_ATTACK
		    || player.getCombatDefinitions().getAttackStyle() == CombatDefinitions.RANGE_ATTACK;
...
//    @Override
//    public boolean canRemoveEquip(int slot, int itemId) { TODO
//	if (ringType == 1) {
