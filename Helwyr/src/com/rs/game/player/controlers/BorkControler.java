...
import com.rs.game.player.content.instancing.bossInstance.MapInstance;
import com.rs.game.player.content.instancing.bossInstance.MapInstance.Stages;
...
public class BorkControler extends Controler {
	...
	if (Utils.currentTimeMillis() - player.getLastBork() <= ((player.isADonator() ? 8 : 12) * 60 * 60 * 1000)) {
	...
	instance.load(new Runnable() {
		...
		FadingScreen.unfade(player, time, new Runnable() {
			...
			bork = new Bork(instance.getTile(12, 15), BorkControler.this);
		...
		try {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				if (!teleported) {
				    ...
				    surokMagis.finish();
				    surokMagis = null;
				    player.getDialogueManager().startDialogue("SurokMagisT", BorkControler.this);
				    player.getPackets().sendBlackOut(0);
