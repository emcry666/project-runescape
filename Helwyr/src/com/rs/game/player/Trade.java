...
public class Trade {
	...
	public void openTrade(Player target) {
		...
		if(player.getSkills().getTotalLevel() < 250) {
			player.getPackets().sendGameMessage("You need a total level of 250 in order to trade.");
			return;
		}
		if(target.getSkills().getTotalLevel() < 250) {
			player.getPackets().sendGameMessage(target.getDisplayName() + " needs a total level of 250 in order to trade.");
			return;
		}
		if(!target.getPlayerPin().hasEnteredPin()) {
			player.getPackets().sendGameMessage("Target has not entered their player pin.");
			return;
		}
		...
		synchronized (this) {
			...
			synchronized (target.getTrade()) {
				...
				if (coinsInInventory < 0) {
					player.sm("<col=ff00ff>If "+target.getUsername()+" accepts the offer he will be recieving over the maximum limit of that item and will reset to 1.");
					target.sm("<col=ff00ff>I have too much of the item id: "+item.getId()+" and will need to bank it.");
					return;
				}
				//int maxAmount = player.getInventory().getItems().getNumberOf(item);
				if (amount < maxAmount)
					...
					item = new Item(item.getId(), maxAmount);
				items.add(item);
				Player.printLog(player, item.getId(), amount, target);
		...
		synchronized (this) {
			synchronized (target.getTrade()) {
				...
				if(CloseTradeStage.DONE != stage) {
					...
					Archiver.writeTradeLog(player, oldTarget,
							player.getTrade().items.toArray(),
							oldTarget.getTrade().items.toArray());
