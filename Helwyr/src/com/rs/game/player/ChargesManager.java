...
public class ChargesManager implements Serializable {
	...
	public ChargesManager(Player player) {
		this.player = player;
	...
	public void process() {
		...
		for (int slot = 0; slot < items.length; slot++) {
			...
			if (ItemConstants.itemDegradesWhileWearing(item.getId()))
				...
				degrade(item.getId(), defaultCharges, slot);
			else if (ItemConstants.itemDegradesWhileCharge(item.getId()))
				degrade2(item.getId(), defaultCharges, slot);
			else if (ItemConstants.itemDegradesWhileHit(item.getId()))
				degrade2(item.getId(), defaultCharges, slot);
		}
	}
	...
	private void degrade2(int itemId, int defaultCharges, int slot) {
		Integer c = charges.remove(itemId);
		if (c == null)
			c = defaultCharges;
		else {
	...
	public void takeCharges(int id, int amount, int wearSlot) {
		int maxCharges = ItemConstants.getItemDefaultCharges(id);
		Integer charge = charges.get(id);
		if (maxCharges == -1) {
			System.out.println("This item cant get charges atm " + id);
			return;
		}
		int amt = charge == null ? amount : charge - amount;
		if (amt <= 0) {
			int newId = ItemConstants.getItemDegrade(id);
			if (newId == -1) {
				if (wearSlot != -1) {
