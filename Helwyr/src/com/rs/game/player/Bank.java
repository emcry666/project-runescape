...
public class Bank implements Serializable {
	...
	private static final long MAX_BANK_SIZE = 762;
	...
	public void openDepositBox() {
		...
		player.getPackets().sendIComponentText(11, 13,
				"Bank Of " + Configs.SERVER_NAME + " - Deposit Box");
...
//		}
		if (!player.hasEnteredPin && player.hasBankPin) {
			...
			player.getPackets().sendRunScript(108, new Object[] { "Enter Your Bank Pin" });
		} else {
			player.getPackets().sendIComponentText(762, 47,
					"Bank Of " + Configs.SERVER_NAME + "");
			player.getInterfaceManager().sendInterface(762);
			player.getInterfaceManager().sendInventoryInterface(763);
			refreshViewingTab();
			refreshTabs();
