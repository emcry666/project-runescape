...
public class PiratesTreasure extends Quest implements Serializable  {
	...
	private QuestJournal journal = new QuestJournal() {
		...
		public QuestJournal display(int stage) {
			...
			if (progress > 6 && progress < 8 && player.hasItem(PiratesTreasure.CHEST_KEY)) {
			...
			} else if(progress > 6 && progress < 8 && !player.hasItem(PiratesTreasure.CHEST_KEY)) {
			...
			if(progress > 8) {
				...
				if(progress > 8 && !player.hasItem(PiratesTreasure.PIRATE_MESSAGE) && player.getQuestManager().get(Quests.PIRATES_TREASURE).getState() != QuestState.COMPLETED) {
