...
public class PrinceAliRescue extends Quest implements Serializable {
	...
	private QuestJournal journal = new QuestJournal() {
		...
		public QuestJournal display(int stage) {
			final int progress = player.getQuestManager().get(Quests.PRINCE_ALI_RESUCE).getStage();
			final QuestState state = player.getQuestManager().get(Quests.PRINCE_ALI_RESUCE).getState();
			switch(progress) {
			/**
			 * Not Started Quest
			 */
	...
	@Override
	public void accept() {
		player.getQuestManager().get(Quests.PRINCE_ALI_RESUCE).setStage(1);
		player.getQuestManager().get(Quests.PRINCE_ALI_RESUCE).setState(QuestState.STARTED);
	}

	@Override
	public void decline() {
		player.getQuestManager().get(Quests.PRINCE_ALI_RESUCE).setStage(0);
		player.getQuestManager().get(Quests.PRINCE_ALI_RESUCE).setState(QuestState.NOT_STARTED);
	}
