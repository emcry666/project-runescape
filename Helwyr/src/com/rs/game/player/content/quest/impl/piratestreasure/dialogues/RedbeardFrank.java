...
public class RedbeardFrank extends Dialogue {
    ...
    public void start() {
        ...
        if(progress > 0 && progress < 7 && stage != 1) {
            ...
            switch(stage) {
                ...
                case -1:
                    if(player.getQuestManager().get(Quests.PIRATES_TREASURE).getStage() < 8 && player.getQuestManager().get(Quests.PIRATES_TREASURE).getStage() > 6) {
                        if (!player.hasItem(PiratesTreasure.CHEST_KEY)) {
                ...
                case 19:
                    sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "Ok thanks, I'll go and get it.", "So why didn't you ever get it?");
                   stage = 20;
                    break;
