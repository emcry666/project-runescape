	...
	public GameTick getGameTick() {
		...
		return new GameTick(2.5) {
			...
			public void run() {
				...
				switch(messageType) {
					case "All 99s":
						bot.publicChat(messages[2][Utils.random(messages[2].length)]);
						break;
					case "Completionist Cape":
						bot.publicChat(messages[1][Utils.random(messages[1].length)]);
						break;
					default:
						bot.publicChat(messages[0][Utils.random(messages[0].length)]);
						break;
				}
			}
