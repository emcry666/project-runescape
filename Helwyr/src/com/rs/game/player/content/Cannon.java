...
public class Cannon implements Serializable
{
	...
	if(!player.canSpawn()) {
		player.getPackets().sendGameMessage("You cannot place a cannon here.");
		return;
	}
	...
	for (NPC n : World.getNPCs()) {
	    ...
	    if (n == null || n.isDead()
	    		|| !n.getDefinitions().hasAttackOption() || n.getPlane() != object.getPlane()) {
	    	continue;
	    }
	    ...
	    if (hit) {
		this.cannonBalls -= 1;
		if(n.getPlane() == getPlayer().getPlane() 
				|| n.getPlane() == object.getPlane())
