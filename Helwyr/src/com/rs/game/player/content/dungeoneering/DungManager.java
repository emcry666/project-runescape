...
public class DungManager implements Serializable {
	...
	public void refreshNames() {
		if (party == null) {
			player.getPackets().sendHideIComponent(939, 59, true);
			player.getPackets().sendHideIComponent(939, 62, true);
			player.getPackets().sendHideIComponent(939, 65, true);
			player.getPackets().sendHideIComponent(939, 68, true);
			player.getPackets().sendHideIComponent(939, 71, true);
			return;
		}
		int index = 0;
		int teamSize = party.getTeam().size();
		for (Player p2 : party.getTeam()) {
			//player.getPackets().sendGlobalString(2376 + (index++), p2.getDisplayName());
			if (index == 0) {
			player.getPackets().sendHideIComponent(939, 59, false);
			player.getPackets().sendIComponentText(939, 59, p2.getDisplayName());
			} else if (index == 1) {
				player.getPackets().sendHideIComponent(939, 62, false);
				player.getPackets().sendIComponentText(939, 62, p2.getDisplayName());
			} else if (index == 2) {
				player.getPackets().sendHideIComponent(939, 65, false);
				player.getPackets().sendIComponentText(939, 65, p2.getDisplayName());
			} else if (index == 3) {
				player.getPackets().sendHideIComponent(939, 68, false);
				player.getPackets().sendIComponentText(939, 68, p2.getDisplayName());
			} else if (index == 4) {
				player.getPackets().sendHideIComponent(939, 71, false);
				player.getPackets().sendIComponentText(939, 71, p2.getDisplayName());
			}
			index++;
		}
		if (teamSize < 5) {
