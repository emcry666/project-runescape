...
import com.rs.game.player.actions.farming.Farming.Herbs;
...
public class CommendationExchange {// 1875 TODO
	...
	public static void handleButtonOptions(Player player, int componentId) {
		if (componentId == 68) {
			...
			if (player.getSkills().getLevelForXp(Skills.FARMING) < 25) {
				player.getPackets()
						.sendGameMessage(
								"You need a farming level of 25 in order to purchase a herblore pack.");
				return;
			} else if (!exchangeCommendation(player, 15))
				return;
			player.getInventory().addItemDrop(
			...
			player.getInventory().addItemDrop(
					...
					Utils.random(2));
			player.getPackets().sendGameMessage(
					"You exchange 15 commendation points for a seed pack.");
		} else {
	...
	private static void addXPForSkill(Player player, int skill, int rate) {
		...
		player.getDialogueManager().startDialogue(
				"SimpleMessage",
				"You gain " + Utils.formatNumber((int) experience)
	...
	private static void addVoidItem(Player player, int index) {
		...
		player.getDialogueManager().startDialogue(
				...
				"You exchange "
						...
						+ ItemDefinitions.getItemDefinitions(voidItem).getName()
								.toLowerCase() + ".", voidItem);
	}
