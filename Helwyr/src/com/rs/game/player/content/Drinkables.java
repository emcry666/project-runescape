...
public final class Drinkables {
	...
	SUPER_SARADOMIN_BREW_FLASK(new int[] { 28227, 28229, 28231, 28233, 28235, 28237 },
	...
	WEAK_STAT_RESTORE_POTION(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE, Skills.MAGIC, Skills.RANGE,
		...
		Skills.FLETCHING, Skills.HERBLORE, Skills.MINING, Skills.RUNECRAFTING, Skills.SLAYER, Skills.SUMMONING,
	...
	WEAK_STAT_CURE_POTION() {
	    ...
	    public void extra(Player player) {
	    	player.addFireImmune(500);
		player.addPoisonImmune(500);
	    }
	},
	...
	WEAK_ARTISTIANS_POTION(Skills.SUMMONING, Skills.CRAFTING, Skills.FLETCHING, Skills.CONSTRUCTION,
		Skills.FIREMAKING) {
	    @Override
	    public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
		int level = actualLevel > realLevel ? realLevel : actualLevel;
	...
	REGULAR_ARTISTIANS_POTION(Skills.SUMMONING, Skills.CRAFTING, Skills.FLETCHING, Skills.CONSTRUCTION,
		Skills.FIREMAKING) {
	    @Override
	    public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
		int level = actualLevel > realLevel ? realLevel : actualLevel;
	...
	STRONG_STAT_CURE_POTION() {
	    @Override
	    public void extra(Player player) {
	    	player.addSuperFireImmune(1200);
		player.addPoisonImmune(2000);
	    }
	},
	...
	STRONG_REJUVINATION_POTION(Skills.SUMMONING, Skills.PRAYER) {
	    ...
	    @Override
	    public void extra(Player player) {
		player.getPrayer().restorePrayer(100 + (player.getSkills().getLevelForXp(Skills.PRAYER) * 10) * 22);
	    }
	},
	...
	STRONG_ARTISTIANS_POTION(Skills.SUMMONING, Skills.CRAFTING, Skills.FLETCHING, Skills.CONSTRUCTION,
		Skills.FIREMAKING) {
	    @Override
	    public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
		int level = actualLevel > realLevel ? realLevel : actualLevel;
	...
	SANFEW_SERUM(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE, Skills.MAGIC, Skills.RANGE, Skills.AGILITY,
		...
		Skills.HERBLORE, Skills.MINING, Skills.RUNECRAFTING, Skills.SLAYER, Skills.SUMMONING, Skills.THIEVING,
	    ...
	    public void extra(Player player) {
		player.getPrayer()
			.restorePrayer((int) ((int) ((player.getSkills().getLevelForXp(Skills.PRAYER) * 0.25 * 10) + 80)
				* player.getAuraManager().getPrayerPotsRestoreMultiplier()));
		player.addPoisonImmune(300);
	    }
	...
	ANTIPOISON() {
	    @Override
	    public void extra(Player player) {
		player.addPoisonImmune(300);// 1 min and half
	    }
	},
	ANTIPOISON_PLUS() {
	    @Override
	    public void extra(Player player) {
		player.addPoisonImmune(900);// like 8 minutes and half
	    }
	},
	ANTIPOISON_DOUBLE_PLUS() {
	    @Override
	    public void extra(Player player) {
		player.addPoisonImmune(1200);// 12 mins
	    }
	},
	SUPER_ANTIPOISON() {
	    @Override
	    public void extra(Player player) {
		player.addPoisonImmune(600);
	    }
	},
	ENERGY_POTION() {
	    @Override
	...
	SARADOMIN_BREW("You drink some of the foul liquid.", Skills.ATTACK, Skills.DEFENCE, Skills.STRENGTH, Skills.MAGIC, Skills.RANGE) {
		...
		@Override
		public void extra(Player player) {
			final int hitpointsModification = (int) (player.getMaxHitpoints() * 0.15);
	...
	OVERLOAD() {
		...
		public boolean canDrink(Player player) {
			if (player.getControlerManager().getControler() instanceof Wilderness || player.getControlerManager().getControler() instanceof CrucibleControler || FfaZone.isOverloadChanged(player)) {
				player.sm("You cannot drink this potion here.");
				return false;
			}
			if (player.getHitpoints() <= 500 || player.getOverloadDelay() > 480) {
				player.sm("You need more than 500 life points to survive the power of overload.");
				return false;
			}
			return true;
		}
	...
	SUPER_PRAYER() {
	    ...
	    public void extra(Player player) {
		player.getPrayer()
			.restorePrayer((int) ((int) (70 + (player.getSkills().getLevelForXp(Skills.PRAYER) * 3.5))
				* player.getAuraManager().getPrayerPotsRestoreMultiplier()));
	    }
	},
	PRAYER_RENEWAL() {
	...
	TANKARD("You quaff the beer. You feel slightly reinvigorated... but very dizzy too.", Skills.ATTACK,
		...
		if (skillId == Skills.STRENGTH) {
		    ...
		    return actualLevel > 9 && actualLevel != 0 ? actualLevel - 9 : actualLevel >> 9;
		}
	    }
	...
	KEG_OF_BEER("You chug the keg. You feel reinvigorated... ...but extremely drunk, too.", Skills.ATTACK,
		...
		if (skillId == Skills.STRENGTH) {
		    ...
		    return actualLevel > 40 && actualLevel != 0 ? actualLevel - 40 : actualLevel >> 40;
		}
	    }
	...
	GROG("You drink a glass of grog.", Skills.ATTACK, Skills.STRENGTH) {
		...
		if (skillId == Skills.STRENGTH) {
		    ...
		    return actualLevel > 6 && actualLevel != 0 ? actualLevel - 6 : actualLevel >> 6;
		}
	    }
	...
	CIDER("You drink a glass of cider.", Skills.ATTACK, Skills.STRENGTH, Skills.FARMING) {
		...
		if (skillId == Skills.FARMING) {
		    ...
		    return actualLevel > 2 && actualLevel != 0 ? actualLevel - 2 : actualLevel >> 2;
		}
	    }
	...
	DWARVEN_STOUT("You drink a glass of Dwarven Stout.", Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE,
	    ...
	    @Override
	    public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
		if (skillId == Skills.MINING && skillId == Skills.SUMMONING) {
		    int boost = (1);
		    int level = actualLevel > realLevel ? realLevel : actualLevel;
		    return level + boost;
		} else {
	...
	DRAGON_BITTER("You drink a glass of Dragon Bitter.", Skills.ATTACK, Skills.STRENGTH) {
		...
		if (skillId == Skills.STRENGTH) {
		    ...
		    return actualLevel > 1 && actualLevel != 0 ? actualLevel - 1 : actualLevel >> 1;
		}
	    }
	...
	RANGERS_AID("You drink a glass of Ranger's Aid.", Skills.RANGE, Skills.DEFENCE) {
		...
		if (skillId == Skills.RANGE) {
		    ...
		    return actualLevel > 5 && actualLevel != 0 ? actualLevel - 5 : actualLevel >> 5;
		}
	    }
	...
	WIZARD_MIND_BOMB("You drink a glass of Wizard's mind bomb.", Skills.ATTACK, Skills.STRENGTH, Skills.MAGIC,
		...
		if (skillId == Skills.MAGIC) {
		    ...
		    return actualLevel > 4 && actualLevel != 0 ? actualLevel - 4 : actualLevel >> 4;
		} else {
		    return actualLevel > 3 && actualLevel != 0 ? actualLevel - 3 : actualLevel >> 3;
		}
	    }
	...
	if (dosesLeft == 0 && drink.isFlask())
	    ...
	    player.getInventory().getItems().set(slot,
		    new Item(dosesLeft > 0 ? drink.getIdForDoses(dosesLeft)
			    : item.getDefinitions().isDungeoneeringItem() ? DUNGEONEERING_VIAL
				    : drink.isPotion() ? VIAL : getReplacedId(drink),
			    1));
	    player.getInventory().refresh(slot);
	}
	for (int skillId : drink.effect.affectedSkills)
	    player.getSkills().set(skillId, drink.effect.getAffectedSkill(player, skillId,
		    player.getSkills().getLevel(skillId), player.getSkills().getLevelForXp(skillId)));
	player.setNextAnimationNoPriority(new Animation((!player.hasNewAnimation() ? 18003 : 18000)));
	player.incrementCount("Potions drank");
	if (drink.isFlask() || drink.isPotion()) {
	    player.getPackets().sendGameMessage(drink.effect.drinkMessage != null ? drink.effect.drinkMessage
	    ...
	    player.getPackets()
		    .sendGameMessage(dosesLeft == 0
			    ? "You have finished your " + (drink.isFlask() ? "flask and the glass shatters to pieces."
				    : drink.isPotion() ? "potion." : item.getName().toLowerCase() + ".")
	...
	public static void resetOverLoadEffect(Player player) {
		...
		player.sm("<col=480000>The effects of overload have worn off and you feel normal again.");
	}
