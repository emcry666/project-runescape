...
import com.rs.game.player.content.IncreasedXpEvent;
...
public class ThroneOfMiscellania implements Serializable {
	...
	public static int getBonusModifier() {
		return (EventManager.eventStarted(EventType.SKILL_EVENT, EventConstants.MISCELLANIA) ? 4 : 3);
	}
		...
		GameEngine.slowExecutor.scheduleWithFixedDelay(new Runnable() {
			...
			public void run() {
				if(!hasVisited) {
					...
					addReputation(getReputation() - 5);
					if(reputation < 0.0)
						reputation = 0.0;
					if(reputation > 5) {
						player.getPackets().sendGameMessage("<img=5><col=ff0000>Your reputation with your kingdom has been decreased by 5%");
					} else {
