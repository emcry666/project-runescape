...
import com.rs.game.player.content.clans.citadel.MemberType;
...
public class ClansManager {
	...
	public static void leaveClanCompletly(Player player) {
		...
		if(player.getControlerManager().getControler() instanceof CitadelControler) {
			player.setNextWorldTile(new WorldTile(Settings.OUTSIDE_CITADEL));
			manager.getClan().getCitadel().removeFromCitadel(MemberType.MEMBER, player);
		}
		player.lock(1);
	...
	public static void openClanDetails(Player player, Player p2, ClansManager manager) {
		...
		player.getPackets().sendIComponentText(1107, 32, "");
		player.getPackets().sendIComponentText(1107, 36, "");
		player.getPackets().sendIComponentText(1107, 34, "");
		player.getPackets().sendIComponentText(1107, 38, "");
