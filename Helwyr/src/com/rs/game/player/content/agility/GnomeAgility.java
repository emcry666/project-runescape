...
public class GnomeAgility {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (!secondLoop) {
					...
					player.setNextWorldTile(toTile);
					player.getSkills().addXp(Skills.AGILITY, 730);
					stop();
					AchievementLogger.handleGnomeAgility(player);
				}
	...
	public static void runGnomeBoard(final Player player, final WorldObject object) {
		...
		player.sm("You skilfully run across the Board");
		WorldTasksManager.schedule(new WorldTask() {
	...
	public static void walkGnomeLog(final Player player) {
		...
		player.sm("You walk carefully across the slippery log...");
		WorldTasksManager.schedule(new WorldTask() {
			boolean secondloop;
			...
			public void run() {
				if (!secondloop) {
					...
					player.sm("... and make it safely to the other side.");
					stop();
				}
			}
		}, 0, 6);
	...
	public static void climbGnomeObstacleNet(final Player player) {
		if (player.getY() != 3426)
			return;
		player.sm("You climb the netting.");
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (!secondloop) {
					...
					player.sm("You passed the obstacle succesfully.");
					stop();
				}
			}
		}, 0, 5);
	...
	public static void enterGnomePipe(final Player player, int objectX, int objectY, WorldObject object) {
		final boolean running = player.getRun();
		player.setRunHidden(false);
		player.lock(8);
		player.addWalkSteps(objectX, objectY == 3431 ? 3437 : 3430, -1, false);
		player.sm("You pulled yourself through the pipes.");
