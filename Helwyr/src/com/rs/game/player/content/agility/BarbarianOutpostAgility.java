...
public class BarbarianOutpostAgility {
			...
			public void run() {
				if(stage == 0) {
					...
					if (getStage(player) == 1) {
						removeStage(player);
						player.getSkills().addXp(Skills.AGILITY, 975);
						AchievementLogger.handleBarbarianAgility(player);
	...
	public static void swingOnRopeSwing(final Player player, final WorldObject object) {
		if(!Agility.hasLevel(player, 35))
			return;
		if(player.getY() != 3554) {
			player.setRouteEvent(new RouteEvent(new WorldTile(2551, 3554, 0), new Runnable() {
	...
	public static void climbOverCrumblingWall(final Player player, final WorldObject object) {
		if (!Agility.hasLevel(player, 35))
			return;
		...
		final WorldTile toTile = new WorldTile(object.getX() + 1, object.getY(),
				object.getPlane());
