...
public class WildernessTeleports {
	 ...
	 */
	public static void preTeleport(final Player player, final WorldObject object) {
		player.sm("You activate the obelisk and hear a faint rumbling sound.");
			...
			public void run() {
				if (timer == 4) {
					if (player.getTeleBlockDelay() > Utils.currentTimeMillis()) {
						player.sm("You have been tele-blocked and are unable to teleport.");
						return;
					}
