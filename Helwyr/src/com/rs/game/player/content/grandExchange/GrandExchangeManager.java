...
public class GrandExchangeManager implements Serializable {
	...
	public GrandExchangeManager(Player player) {
		this.player = player;
		offerUIds = new long[6];
		history = new OfferHistory[5];
	}

	public void setOfferUIDS(long[] uids) {
		this.offerUIds = uids;
	}

	public void setOfferHistory(OfferHistory[] history) {
	...
	public void openGrandExchange() {
		/*if (player.getPlayerPin().hasPin() && !player.getPlayerPin().hasEnteredPin()) {
	...
	public int getItemAmount(Item item) {
		...
		return player.getInventory().getNumerOf(item.getId())
				+ player.getInventory().getNumerOf(notedId);
	}
	...
	public void sendInfo(Item item) {
		...
		bonuses[CombatDefinitions.ABSORVE_MELEE_BONUS] += defs
				.getAbsorveMeleeBonus();
		bonuses[CombatDefinitions.ABSORVE_MAGE_BONUS] += defs
				.getAbsorveMageBonus();
		bonuses[CombatDefinitions.ABSORVE_RANGE_BONUS] += defs
				.getAbsorveRangeBonus();
		...
		if (hasBonus) {
			 ...
			 * "<br>Worn on yourself"); player.getPackets().sendGlobalString(35,
			 * "<br>Attack<br><col=ffff00>+" +
			 * bonuses[CombatDefinitions.STAB_ATTACK] + "<br><col=ffff00>+" +
			 ...
			 * bonuses[CombatDefinitions.RANGE_ATTACK] + "<br><col=ffff00>---" +
			 * "<br>Strength" + "<br>Ranged Strength" + "<br>Magic Damage" +
			 * "<br>Absorve Melee" + "<br>Absorve Magic" + "<br>Absorve Ranged"
			 * + "<br>Prayer Bonus"); player.getPackets().sendGlobalString(36,
			 ...
			 * bonuses[CombatDefinitions.STAB_DEF] + "<br><col=ffff00>+" +
			 * bonuses[CombatDefinitions.SLASH_DEF] + "<br><col=ffff00>+" +
			 * bonuses[CombatDefinitions.CRUSH_DEF] + "<br><col=ffff00>+" +
			 * bonuses[CombatDefinitions.MAGIC_DEF] + "<br><col=ffff00>+" +
			 * bonuses[CombatDefinitions.RANGE_DEF] + "<br><col=ffff00>+" +
			 ...
			 * "%<br><col=ffff00>" + bonuses[CombatDefinitions.PRAYER_BONUS]);
			 */
		} else {
			player.getPackets().sendGlobalString(26, "");
	...
	public void confirmOffer() {
		...
		if (buy) {
			...
			if (!player.getInventory().hasEnoughCoins(price)) {
				player.getPackets().sendGameMessage(
						"You do not have enough coins to cover the offer.");
				return;
			}
			if (itemId == 1464) {
				player.getPackets()
						.sendGameMessage(
								"You cannot buy 100M tickets in the Grand Exchange, please use the General Store.");
				return;
			}
			if(defs.getValue(itemId) < 2) {
				player.getPackets().sendGameMessage("You cannot make this purchase.");
				return;
			}
			if(!player.getInventory().checkMoneyAndRemove(price))
				return;
		} else {
	...
	public void makeOffer(int slot, boolean sell) {
		if (!isSlotFree(slot) || getCurrentSlot() != -1) 
			return;
		if(sell && player.getRights() > 1 && !Settings.isOwner(player))
			return;
		setType(sell ? 1 : 0);
		setSlot(slot);
		if (sell) {
			...
			player.getInterfaceManager().setInterface(true, 752, 7, 389);
			player.getPackets()
					.sendRunScript(570, "Grand Exchange Item Search");
		}
	}
