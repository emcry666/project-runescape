...
public class Offer extends Item {
    ...
    public Offer(int id, int ammount, int price, boolean buy,
	    boolean isAutoBuy, long uid, String ownerName) {
    ...
    public Offer(int id, int ammount, int price, String name, boolean buy,
	    boolean isAutoBuy, long uid, ItemsContainer<Item> receivedItems,
	    int totalPriceSoFar, int totalAmmountSoFar, boolean canceled,
	    String ownerName) {
	// item id buying or selling
	// total amount buying or selling
	super(id, ammount);
	...
	this.offerUid = uid;
	this.setOwnerId(ownerName);
	this.totalAmmountSoFar = totalAmmountSoFar;
	...
	Player fromPlayer = World.getPlayer(owner.getUsername());
	/*if (fromPlayer != null) {
	    GrandExchange.linkOffers(fromPlayer);
	}*/
	...
	if (item == null)
	    return false;
	if (item.getId() == 995
		&& owner.getMoneyPouch().getCoinsAmount()
		+ item.getAmount() > ((long) 2147483647 * 2)) {
	    owner.getPackets().sendGameMessage(
		    "Not enough space in your inventory.");
	    return false;
	}
	...
	if (!defs.isStackable() && option == (item.getAmount() == 1 ? 0 : 1)) {
	    ...
	    owner.getInventory().addItem(new Item(defs.getCertId() != -1 ? defs.getCertId() : item.getId(), item.getAmount()));
    ...
    public void updateOfferFromAutoBuy() {
	int left = getAmount() - totalAmmountSoFar;
	if (left <= 0) {
	    return;
	}
	int exchangeAmt = Utils.random(1,
		AutoBuyStock.getStockForItem(getId()) - 1);
	if (exchangeAmt > left) {
	    exchangeAmt = left;
	}
	if (exchangeAmt <= 0) {
	...
	// System.out.println(""+exchangeAmt+" - "+getId());
	int totalPrice = exchangeAmt * GrandExchange.getPrice(getId());
	int leftcoins = exchangeAmt * price - exchangeAmt
		* GrandExchange.getPrice(getId());
	if (leftcoins > 0) {
	    receivedItems.add(new Item(995, leftcoins));
	}
	receivedItems.add(new Item(getId(), exchangeAmt));
	AutoBuyStock.decreaseStock(getId(), exchangeAmt);
	totalAmmountSoFar += exchangeAmt;
	totalPriceSoFar += totalPrice;
		update();
    }
