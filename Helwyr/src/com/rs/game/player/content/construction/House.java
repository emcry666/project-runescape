...
public class House implements Serializable {
	...
	public void openRoomCreationMenu(int roomX, int roomY, int plane) {
		...
		if (room != null) {
			...
			for (int index = 0; index < HouseConstants.Room.values().length - 2; index++) {
				...
				if (player.getSkills().getLevel(Skills.CONSTRUCTION) >= refRoom.getLevel() && !player.getInventory().hasEnoughCoins(refRoom.getPrice())) {
					player.getPackets().sendIComponentText(402, index + (refRoom == HouseConstants.Room.DUNGEON_STAIRS || refRoom == HouseConstants.Room.DUNGEON_PIT ? 69 : refRoom == HouseConstants.Room.TREASURE_ROOM ? 70 : 68),
	...
	public void createRoom(int slot) {
		...
		if (!player.getInventory().hasEnoughCoins(room.getPrice()) && !player.isALegendDonator()) {
	...
	public void createRoom(RoomReference room) {
		if (!loaded)
			return;
		if (!player.getInventory().hasEnoughCoins(room.room.getPrice()) && !player.isALegendDonator()) { // better
			player.getPackets().sendGameMessage("You don't have enough coins to build this room.");
			return;
		}
	...
	public void openBuildInterface(WorldObject object, final Builds build) {
		...
		for (int index = 0; index < build.getPieces().length; index++) {
			...
			itemArray[interId == 396 ? BUILD_INDEXES[index] : index] = new Item(
					piece.getItemId(), 1);
			player.getVarsManager().sendVar(1485 + index,player.getSkills().getLevel(Skills.CONSTRUCTION) >= piece
							.getLevel()
		...
		for (int i = 0; i < (interId == 396 ? 7 : 3); i++) {
			if (i >= build.getPieces().length) {
				...
				if (ItemDefinitions
						.getItemDefinitions(build.getPieces()[i].getItemId())
						.getName().toLowerCase().contains("marble portal")) {
					int finalC = 14 + (5 * i);
					String portalName = "Portal";
					if (finalC == 14) {
						portalName = "Varrock Portal";
					} else if (finalC == 19) {
						portalName = "Lumbridge Portal";
					} else if (finalC == 24) {
						portalName = "Falador Portal";
					} else if (finalC == 29) {
						portalName = "Camelot Portal";
					} else if (finalC == 34) {
						portalName = "Argougne Portal";
					} else if (finalC == 39) {
						portalName = "Yanille Portal";
					} else if (finalC == 44) {
						portalName = "Kharyrll Portal";
					}
					player.getPackets().sendIComponentText(interId, finalC,
							portalName);
				} else {
					player.getPackets()
							.sendIComponentText(
	...
	public boolean hasWaterCan() {
		for (int id = 5333; id <= 5340; id++)
			if (player.getInventory().containsItem(id))
				return true;
		return false;
	}
	...
	public boolean joinHouse(final Player player) {
		...
		if (loaded) {
			...
			WorldTasksManager.schedule(new WorldTask() {
			 ...
			 WorldTasksManager.schedule(new WorldTask() {//maybe an issue -yamz
				@Override
				public void run() {
	...
	public void refreshNumberOfRooms() {
		player.getPackets().sendGlobalConfig(944, roomsR.size());//SENDCSVarInteger
	}
			...
			for (int x = 0; x < data[plane].length; x++) {
				for (int y = 0; y < data[plane][x].length; y++) {
					if (data[plane][x][y] != null)
						...
						RegionBuilder.copyChunk(HouseConstants.BLACK[0], HouseConstants.BLACK[1], 0, boundChuncks[0] + x,
						...
						RegionBuilder.copyChunk(HouseConstants.DUNGEON[0] + (look >= 4 ? 8 : 0), HouseConstants.DUNGEON[1],
								look & 0x3, boundChuncks[0] + x, boundChuncks[1] + y, plane, 0);
					else
