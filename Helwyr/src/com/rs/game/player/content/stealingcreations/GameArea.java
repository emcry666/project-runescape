...
public class GameArea {
	...
	public void calculate() {
		...
		final int total = flags.length * flags.length;
		int skillPlots = (int) (total * 0.3F);
		int obstacles = (int) (total * 0.2F);

		while (skillPlots-- > 0) {
			setRandom(100, 0, 0, flags.length, flags.length,
					Utils.random(4) + 9, Utils.random(4) == 0 ? Utils.random(3)
							: Utils.random(6), Utils.random(4), 60);
		}

		while (obstacles-- > 0) {
			final int type = Utils.random(5) + 3;
			final int rotation = type == 5 ? 3 : Utils.random(4);
	...
	public void create() {
		...
		final int b = RegionBuilder.findEmptyRegionHash(flags.length,
				flags.length);
		base = new int[] { (b >> 8) << 6, (b & 0xFF) << 6 };
		System.err.println("Base:" + base[0] + "," + base[1] + "," + b);
		for (int x = 0; x < flags.length; x++) {
			for (int y = 0; y < flags.length; y++) {
				final int chunkX = (base[0] >> 3) + x;
				final int chunkY = (base[1] >> 3) + y;
				final int type = getType(x, y);
				final int rot = getRotation(x, y);
				final int tier = getTier(x, y);
				int[] copy;
				switch (type) {
				...
				if (type >= 9 && type <= 12 && tier > 0) {
					...
					copy = EMPTY;
				RegionBuilder.copyChunk(copy[0], copy[1], 0, chunkX, chunkY, 0,
						rot);
			}
		}
		...
		final int managerBlue = Helper.MANAGER_NPCS[Utils
				.random(Helper.MANAGER_NPCS.length)];
		final int managerRed = Helper.MANAGER_NPCS[Utils
				.random(Helper.MANAGER_NPCS.length)];
		...
		World.spawnNPC(
				...
				-1, false, true).setRandomWalk(false);
	}
	...
	public void destroy() {
		...
		RegionBuilder.destroyMap(base[0] >> 3, base[1] >> 3, flags.length,
				flags.length);
		base = null;
	...
	private boolean setRandom(int attempts, int minX, int minY, int maxX,
		...
		while (attempts-- > 1) {
			final int x = minX + Utils.random(maxX - minX);
			final int y = minY + Utils.random(maxY - minY);
			if (getType(x, y) == 2) {
