...
public class Helper {
	...
	public static final int[] CLASS_ITEMS_BASE = { 14132, 14122, 14142, 14152, 14172, 14162, 14367, 14357, 14347, 14411, 14391, 14401, 14337, 14317, 14327, 14297, 14287, 14307, 14192, 14202, 12850, 12851, 14422, 14377, 14421, -1, -1, 14215, 14225, 14235, 14245, 14255, 14265, 14275, 14285 };
	...
	public static int getItemIndex(String name) {
		if (name.contains("rune") || name.contains("staff"))
			...
			return 2;
		else if (name.contains("robe") || name.contains("magic hat"))
			return 8;
		else if (name.contains("hardbody") || name.contains("coif") || name.contains("chaps"))
			return 6;
	...
	public static boolean proccessKilnItems(Player player, int componentId, int index, int itemId, int amount) {
		...
		if (amount > 0 && player.getInventory().containsItem(clayId, amount)) {
			...
			if (!player.getInventory().containsItem(clayId, amount))
				setKilnQuality(player);
			return true;
		} else {
	...
	public static void awardPoints(Player player, Score personal, int winner) {
		...
		player.setStealingCreationPoints(player.getStealingCreationPoints() + points);
		player.setStealingCreationGamesWon(player.getStealingCreationGamesWon() + 1);
		player.getPackets().sendGameMessage("You earn: " + points + " reward point" + (points != 1 ? "s" : "") + ".");
	}

	public static void displayScores(Player player, Score personal, List<Score> total, int winner, boolean onBlueTeam) {
		...
		player.getPackets().sendGlobalConfig(597, onBlueTeam ? blueTotal : redTotal); // your total
		player.getPackets().sendGlobalConfig(598, onBlueTeam ? redTotal : blueTotal); // enemy total
		/*player.getPackets().sendGlobalConfig(597, redTotal); // your total
		player.getPackets().sendGlobalConfig(598, blueTotal); // enemy total*/
		// personal
		 ...
		 * Winning
		 */
		player.getPackets().sendGameMessage("Blue Team (1) Score: " + blueTotal + " - Red Team (2) Score: " + redTotal);
		if (winner != 0 && winner == (personal.getTeam() ? 2 : 1)) {
			...
			player.setStealingCreationPoints(player.getStealingCreationPoints() + 1);

			int rate = (player.isADonator() ? 40 : 50);

			if(EventManager.eventStarted(EventType.MINIGAME_EVENT, EventConstants.STEALING_CREATIONS))
				rate = rate - 5;
			
			int randomS = Utils.random(rate);
			int lNumber = Utils.random(rate);
			int randomS2 = Utils.random(rate / 2);
			int lNumber2 = Utils.random(rate / 2);

			if(randomS == lNumber) {
				if(Utils.random(2) == 0) {
					if(player.getInventory().getFreeSlots() < 1) {
						player.getBank().addItem(21467, 1, false);
					} else {
						player.getInventory().addItem(21467, 1);
					}
				} else if(Utils.random(1) == 0) {
					if(player.getInventory().getFreeSlots() < 1) {
						player.getBank().addItem(21468, 1, false);
					} else {
						player.getInventory().addItem(21468, 1);
					}
				} else {
					if(player.getInventory().getFreeSlots() < 1) {
						player.getBank().addItem(21469, 1, false);
					} else {
						player.getInventory().addItem(21469, 1);
					}
				}
				World.sendWorldMessage("<im=5><col=00FF00>" + player.getDisplayName() + " has just recieved a Trickster Piece from Stealing Creations!", false);
			} else {
				if(randomS2 == lNumber2) {
					if(Utils.random(1) == 0) {
						if(player.getInventory().getFreeSlots() < 1) {
							player.getBank().addItem(21470, 1, false);
						} else {
							player.getInventory().addItem(21470, 1);
						}
					} else {
						if(player.getInventory().getFreeSlots() < 1) {
							player.getBank().addItem(21471, 1, false);
						} else {
							player.getInventory().addItem(21471, 1);
						}
					}
					World.sendWorldMessage("<im=5><col=00FF00>" + player.getDisplayName() + " has just recieved a Trickster Piece from Stealing Creations!", false);
				}
			...
			int rate = (player.isADonator() ? 65 : 75);
		...
		player.getPackets().sendGameMessage("Individual Total Score: " + personal.total((personal.getTeam() ? 2 : 1) == winner) + " points.");

	}

	public static void displayKiln(Player player) {
	...
	public static void setKilnQuality(Player player) {
		int quality = 0;
		...
		if (!player.getInventory().containsOneItem(SACRED_CLAY)) {//when finished processing
			player.getPackets().sendGameMessage("You've processed the last of your clay!");
			player.stopAll();
			return;
		}
		setKilnQuality(player, quality);
	}

	public static void setKilnQuality(Player player, int quality) {
	...
	public static void refreshKiln(Player player) {
		...
		for (int i = 0; i < 5; i++)
			player.getPackets().sendGlobalConfig(KILN_CONFIG_BASE + i, player.getInventory().getAmountOf(SACRED_CLAY[i]));
		for (int i = 0; i < 5; i++)
