...
public class MoneyPouch implements Serializable {
	...
	public void withdrawPouch() {
		...
		if (player.getTrade().isTrading()) {
			player.sm("You cannot perform this action whilst trading.");
			return;
		}
	...
	public boolean addToMoneyPouch(int amount) {
		return sendDynamicInteraction(amount, false, TYPE_INV);
	}
	
	public boolean removeFromMoneyPouch(int amount) {
		return sendDynamicInteraction(amount, true, TYPE_INV);
	}
