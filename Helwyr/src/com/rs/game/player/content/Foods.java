...
public class Foods {
	...
	public static enum Food {
		...
		SWEETCORN(5988, 0, Effect.SWEETCORN),
		...
		HEIM_CRAB(18159, 0, Effect.HEIM_CRAB),
		
		BLUE_CRAB(18175, 0, Effect.BLUE_CRAB),
		
		BOULDABASS(18171, 0, Effect.BOULDABASS),
		
		CAVE_MORAY(18177, 0, Effect.CAVE_MORAY),
		
		DUSK_EEL(18163, 0, Effect.DUSK_EEL),
		
		GIANT_FLATFISH(18165, 0, Effect.GIANT_FLATFISH),
		
		RED_EYE(18161, 0, Effect.RED_EYE),
		
		SALVE_EEL(18173, 0, Effect.SALVE_EEL),
		
		SHORT_FINNED_EEL(18167, 0, Effect.SHORT_FINNED_EEL),
		
		WEB_SNIPER(18169, 0, Effect.WEB_SNIPER),
		...
		ROCK_CAKE(7509, -1, Effect.ROCK_CAKE),
	...
	public static enum Effect {
		...
		KEBAB {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				int random = Utils.random(0, 1000);
				if (random >= 0 && random < 100) {
					player.sm("That kebab didn't seem to do a lot.");
				} else if (random >= 100 && random < 500) {
					player.sm("It restores some life points.");
					int hp = player.getMaxHitpoints();
					player.heal(hp / 10);
				} else if (random >= 500 && random < 950) {
					player.sm("That was a good kebab.");
					int randomHeal = Utils.random(100, 200);
					player.heal(randomHeal);					
				} else if (random >= 950 && random <= 1000) {
					player.sm("Wow, that was an amazing kebab! You feel really invigorated.");
					int randomHeal = Utils.random(200, 300);
					player.heal(randomHeal);
				}
			}
		},
		...
		ONION_MESSAGE {
			...
			public void effect(Object object) {
				...
				if (player.getAppearence().isMale()) {
					player.getPackets().sendGameMessage("It hurts to see a grown male cry.");
				} else {
					player.getPackets().sendGameMessage("It hurts to see a grown female cry.");
				}
			}
		},
		...
		HEIM_CRAB {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.applyHit(new Hit(player, 20,HitLook.HEALED_DAMAGE));
			}
		},
		...
		BOULDABASS {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.applyHit(new Hit(player, 170,HitLook.HEALED_DAMAGE));
			}
		},
		...
		BLUE_CRAB {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.applyHit(new Hit(player, 220,HitLook.HEALED_DAMAGE));
			}
		},
		...
		CAVE_MORAY {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.applyHit(new Hit(player, 250,HitLook.HEALED_DAMAGE));
			}
		},
		...
		DUSK_EEL {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.applyHit(new Hit(player, 70,HitLook.HEALED_DAMAGE));
			}
		},
		...
		SALVE_EEL {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.applyHit(new Hit(player, 200,HitLook.HEALED_DAMAGE));
			}
		},
		...
		SHORT_FINNED_EEL {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.applyHit(new Hit(player, 120,HitLook.HEALED_DAMAGE));
			}
		},
		...
		WEB_SNIPER {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.applyHit(new Hit(player, 80,HitLook.HEALED_DAMAGE));
			}
		},
	...
	public static final int getCookingLevel(int id) {
		...
		return defs.isNoted() ? 0 : defs.getCSOpcode(2951);
	}
