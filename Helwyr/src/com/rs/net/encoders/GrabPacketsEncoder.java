...
public final class GrabPacketsEncoder extends Encoder {
	...
	public void sendCacheArchive(int indexId, int containerId, boolean priority)
		...
		if (getRequests().containsKey(indexId + (containerId << 8))) {
			amt = getRequests().get(indexId + (containerId << 8));
			if (++amt > 4) {
				System.out.println("Multiple requets for index " + indexId
					+ " from " + session.getLocalAddress() + "!");
				return;
			}
		}

		getRequests().put(indexId + (containerId << 8), amt);
		...
		if (indexId == 255 && containerId == 255) {
			...
			// May or may not prevent invalid archive requests, just thought I'd
			// make use of these methods
			/*
			 * if (Cache.STORE.getIndexes()[indexId] == null ||
			 * !Cache.STORE.getIndexes()[indexId].archiveExists(containerId)) {
			 * return; }
			 */
			ChannelBuffer buffer = getArchivePacketData(indexId, containerId,
				priority);
			if (buffer == null) {
				return;
			}
