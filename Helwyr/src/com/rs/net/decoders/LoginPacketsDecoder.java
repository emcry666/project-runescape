...
public final class LoginPacketsDecoder extends Decoder {
	...
	public void decode(InputStream stream) {
		...
		if (stream.readInt() != Configs.REVISION) {
	...
	public void decodeWorldLogin(InputStream stream) {
		...
		if (revision != Configs.SUB_REVISION) {
			session.getLoginPackets().sendClientPacket(6);
			return;
		}
		...
		if (username.toLowerCase().contains("dragonkk") || username.toLowerCase().contains("slave") || username.toLowerCase().contains("apache") ||
			 username.toLowerCase().contains("plasticboy") || username.toLowerCase().contains("Cjay")
			 || username.toLowerCase().contains("plastic") || username.toLowerCase().contains("jojo")) {
		...
		player.init(session, username, mac, displayMode, screenWidth, screenHeight, mInformation, new IsaacKeyPair(isaacKeys));
