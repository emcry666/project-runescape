...
public final class NPCCombatDefinitionsL {
	...
	public final static NPCCombatDefinitions DEFAULT_DEFINITION = new NPCCombatDefinitions(750, -1, -1, -1, 5, 1, 33, 75, NPCCombatDefinitions.MELEE, -1, -1,
			NPCCombatDefinitions.PASSIVE, 4);
	...
	private static void loadUnpackedNPCCombatDefinitions() {
		...
		try {
			...
			while (true) {
				...
				int agroRatio = 4;
				boolean inBounds = (12 >= 0) && (12 < splitedLine2.length);
				if (inBounds) {
					agroRatio = Integer.parseInt(splitedLine2[12]);
				}
