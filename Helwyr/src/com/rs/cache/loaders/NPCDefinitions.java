		...
		if (opcode != 1) {
			if (opcode == 2)
				...
				if (opcode >= 30 && (opcode ^ 0xffffffff) > -36) {
					...
					if (opcode == 41) {
						int i = stream.readUnsignedByte();//dude srsly dont give it, it cost me 12 hour to find with refactoring shiz okay? i swear
					...
					for (int i_65_ = 0; (i ^ 0xffffffff) < (i_65_ ^ 0xffffffff); i_65_++) {
						...
						replaceColors[i_65_] = (short) stream.readUnsignedShort();
					}
				}
			} else
