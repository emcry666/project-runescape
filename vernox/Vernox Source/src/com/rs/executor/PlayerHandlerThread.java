...
public final class PlayerHandlerThread extends Thread {
	...
	private final void initPlayer(Player player, LoginDetails details) {
		try {//not sure why its erroring, when i converted the changes back.. asdf i do ir may i type something? sure.
			player.init(details.session, details.lobby, details.username, details.displayName, details.macAddress, details.email, details.rights, details.messageIcon, details.masterLogin, details.donator, details.extremeDonator, details.support, details.gfxDesigner, details.muted, details.messageIcon, details.displayMode, details.screenWidth, details.screenHeight, details.machineInfo, new IsaacKeyPair(details.sessionKeys));
			if (details.lobby) {
