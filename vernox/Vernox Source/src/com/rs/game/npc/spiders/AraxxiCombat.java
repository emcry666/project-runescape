		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				for (Entity t : players) {
					int damage = getMaxHit(npc, Utils.random(2500,4500), NPCCombatDefinitions.MELEE, t);
	...
	private void sendRange(NPC npc, Boolean anim, Entity farthest) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				for (Entity t : players) {
					int damage = getMaxHit(npc, Utils.random(2500, 3750), NPCCombatDefinitions.RANGE, t);
	...
	private void sendMagic(NPC npc, Boolean anim, Entity farthest) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				for (Entity t : players) {
					int damage = getMaxHit(npc, Utils.random(2500, 3500), NPCCombatDefinitions.MAGE, t);
			...
			public void run() {
				if (loop == 2) {
					if(target.getX() >= npc.getX() -3 && target.getX() <= npc.getX() + 3
						...
						target.applyHit(new Hit(npc,  Utils.random(7500, 9500), HitLook.REGULAR_DAMAGE));
