...
public class Appearence implements Serializable {
	...
	public void generateAppearenceData() {
		...
		if(showSkillLevel)
			flag |= 0x4;
		if (title != 0)
			flag |= title >= 32 && title <= 37 || title == 43 || title == 40 || title >= 59 && title <= 67 || title >= 45 && title <= 47  ? 0x80 : 0x40; // after/before
			stream.writeByte(flag);
			if (title != 0) {
				String titleName =
						title == 5611 ? player.getAppearence().isMale() ? "<img=11><col=7e7e7e>Ironman</col> " : 
						    "<img=11><col=7e7e7e>Ironwoman</col> " :
							title == 5612 ? "<img=9><col=7e7e7e>Iron</col><col=009fff>Donator</col> " :
							title == 5613 ? "<img=9><col=009fff>Donator</col> " :
							title == 6611 ? "<img=8><col=7F00FF>Extreme Donator</col> " :
							title == 5614 ? "<img=1><col=fdff00>Founder</col> " :
							title == 5615 ? "<img=1><col=fdff00> Developer</col> " :
							title == 5616 ? "<img=10><col=ccccc> Web Developer</col> " :
