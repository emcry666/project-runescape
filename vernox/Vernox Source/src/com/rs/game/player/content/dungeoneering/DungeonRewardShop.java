...
public class DungeonRewardShop {
	...
	public static void purchase(Player player) {
		...
		player.setDungeoneeringTokens(-map.getIntValue(1072));
		refreshPoints(player);
	}
	...
	private static boolean canPurchase(Player player) {
		...
		if (player.getSkills().getLevel(skill) < levelRequirement || player.getSkills().getLevel(Skills.DUNGEONEERING) < dungeoneeringLevel || player.getDungeoneeringTokens() < price)
	...
	private static void refreshPoints(Player player) {
		player.getPackets().sendIComponentText(REWARD_SHOP, 31, "" + player.getDungeoneeringTokens());
	}
}
