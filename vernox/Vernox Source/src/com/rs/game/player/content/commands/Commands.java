...
public final class Commands {
	...
	case "resetgrimtask":
	    player.setTask(null);
	    player.getPackets().sendGameMessage("nulled task");
	    return true;
	...
	case "givemod":
		...
		if (target == null) {
			target = World.getPlayerByDisplayName(name);
			if (target != null) {
				target.setDisplayName(Utils.formatPlayerNameForProtocol(name));
			}
			loggedIn = false;
		}
		if (target == null) {
	...
	case "givedonator":
	    ...
	    target.getAppearence().setTitle(5613);
	    target.isDonor = true;
	    if (loggedIn2) {
		target.getPackets().sendGameMessage(
	...
	case "giveextremedonator":
	    ...
	    target.isExtrDonor = true;
	    target.getAppearence().setTitle(6611);
	    if (loggedIn25) {
		target.getPackets().sendGameMessage(
			"You have been promoted by " + Utils.formatPlayerNameForDisplay(player.getUsername()) + ".",
			true);
	...
	if (clientCommand) {
		...
		return true;
	    case "forceitem"://forceitem playername id amount
		name = cmd[1];
		...
		return true;
	    case "addwpoints":
		player.setWarriorPoints(3, 3000);
		player.sendMessage("you now have " +player.getWarriorPoints());
		return true;
	    case "testdeath":
		...
		for (int i = 0; i < 8; i++)
		    player.getCombatDefinitions().getStats()[i] = 500000;
		return true;
	    ...
	    case "savecoords":
		  ...
		  clipboard2.setContents(selection2, selection2);
		  String coords = ("Coords: " + player.getX() + ", " + player.getY() + ", "
		   + player.getPlane() + ", regionId: " + player.getRegionId() + ", rx: " + player.getChunkX()
		   + ", ry: " + player.getChunkY() + ", int: " + player.getTileHash());
		  try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("C:/Users/morterlife/VernoxSavedCoords/SavedCoords.txt", true)))) 
		  {
		      out.println("------------------------------------------------------------------------");
		      out.println(name + " - " + coords);
		      player.getPackets().sendPanelBoxMessage("Saved coords!");
		  }
		  catch (IOException e) 
		  {
		      player.getPackets().sendPanelBoxMessage("ERROR: can NOT save coords.");
		  }
		  return true;
	    case "ccoords":
...
//		    p.sendMessage("<col=ff0000>Server is getting updated, everyone getting teleported to home!</col>");
//		}
	...
	if (clientCommand) {
	    ...
	    case "dz":
	    case "donatorzone":
		if (player.isDonor == true || player.isExtrDonor == true) {
	...
	if (player.getUsername().equalsIgnoreCase("Mystic_Wolf") 
		|| player.getUsername().equalsIgnoreCase("life"))
		//|| player.getUsername().equalsIgnoreCase("Wizard"))
	{
	    World.sendIgnoreableWorldMessage(player, "<img=1><col=00ff00>"
                        + player.getDisplayName() + "</col>: "
                        + message + "</col>", false);
	}
	if (player.isDonor == true) {
	    World.sendIgnoreableWorldMessage(player,
		    "<col=4169e1>" + player.getDisplayName() + ": " + message + "</col>", false);
	}
	if (player.isExtrDonor == true)
	{
	    World.sendIgnoreableWorldMessage(player,
		    "<col=7F00FF>" + player.getDisplayName() + ": " + message + "</col>", false);
	}
	...
	if (clientCommand) {
		...
		if(player.isDonor == true)
		{
		    player.isDonor = false;
		    player.getPackets().sendGameMessage("false");
		}
		else {
		    player.isDonor = true;
		...
		break;
	    case "blueskin":
		if (!player.isDonor || !player.isExtrDonor) {
		    player.getPackets().sendGameMessage("You do not have the privileges to use this.");
		    return true;
		}
		...
		return true;
	    case "home":
		Magic.sendLunarTeleportSpell(player, 0, 0.0D, Settings.START_PLAYER_LOCATION);
		break;
	    case "dz":
	    case "donatorzone":
		if (player.isDonor == true || player.isExtrDonor == true) {
		...
		return true;
	    case "geticons":
		for (int i = 0; i < 20;i++) {//use this to get the icons outa the cahce. as some people may not have a 831 cache editor. but u can also do this
		    player.getPackets().sendGameMessage("IconId: " + i + " :<img="+ i + "> ");
		}
		break;
	    case "help":
		...
		return true;
	    case "livechat":
		player.getPackets().sendOpenURL("http://www.vernox3.com/site/index.php?page=donate_other");
		return true;
	    case "reportplayer":
		player.getPackets().sendOpenURL("http://www.vernox3.com/forms.php?do=form&fid=4");
		return true;
	    case "reportbug":
		player.getPackets().sendOpenURL("http://www.vernox3.com/misc.php?do=form&fid=3");
		return true;
	    case "recoverpass":
		player.getPackets().sendOpenURL("http://www.vernox3.com/misc.php?do=form&fid=1");
		return true;
	    case "appealoffence":
		player.getPackets().sendOpenURL("http://www.vernox3.com/misc.php?do=form&fid=2");
		return true;
	    case "checkreport":
		player.getPackets().sendOpenURL("http://www.vernox3.com/forumdisplay.php?153-Player-reports");
		return true;
	    case "checkappeal":
		player.getPackets().sendOpenURL("http://www.vernox3.com/forumdisplay.php?142-Offence-Appeal");
		return true;
	    case "rules":
		player.getPackets().sendOpenURL("http://www.vernox3.com/announcement.php?f=52&a=32");
		return true;
	    case "guides":
		player.getPackets().sendOpenURL("http://www.vernox3.com/showthread.php?61570-Guide-of-Guides");
		return true;
	    case "thread":// REQUESTED BY BO, DONT YELL AT CJAY FOR IT K9K
		player.getPackets()
			.sendOpenURL("http://www.vernox3.com/showthread.php?" + Integer.parseInt(cmd[1]));
		return true;
	    case "yell":
		message = "";
		for (int i = 1; i < cmd.length; i++)
