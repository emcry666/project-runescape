...
public class Player extends Entity {
    ...
    public double getDungeoneeringTokens() {
	return dungeoneeringTokens;
    }


    public void setDungeoneeringTokens(double dungeoneeringTokens) {
    ...
    public double dungeoneeringTokens = 0;
	...
	if (starter == 0)
	{
	    getDialogueManager().startDialogue("Starter");
	}
	
	if(isWeekend())
	{
	    getPackets().sendGameMessage("<img=6><col=ffae42>Double exp weekend is currently active!");
	}
	...
	if (isIronMan == true)
	{
	    getPackets().sendGameMessage("<col=7e7e7e>You're playing as an ironman</col>");
	    getAppearence().setTitle(5611);
	}
	
	if (isDonor == true && isIronMan == true)
	{
	    getAppearence().setTitle(5612);
	}
	
	if (isDonor == true && isIronMan != true)
	{
	    getAppearence().setTitle(5613);
	}
	
	if (isExtrDonor == true && isIronMan == true) {
	    getAppearence().setTitle(6611);
	}
	...
	if (getUsername().equalsIgnoreCase("Mystic_Wolf"))// || getUsername().equalsIgnoreCase("Life"))
	{
	    getAppearence().setTitle(5614);
	}
	if (getUsername().equalsIgnoreCase("Life"))
	{
	    getAppearence().setTitle(5615);
	}
	if(getUsername().equalsIgnoreCase("Austin"))
	{
	    getAppearence().setTitle(5616);
	}
	
	if (getRights() > 1)
	{
	    sendMessage("<img=1>"+getDisplayName() + " has logged in!");
	}
	...
	try{
	    ...
	    String secret = "101951fe7ebe7bd8c77d14f75746b4bc"; //YOUR SECRET KEY!
		String email = "dylandoldersum@outlook.com"; //This is the one you use to login into RSPS-PAY
				...
				String[] ary = results.split(",");
					 for(int i = 0; i < ary.length; i++){
						switch(ary[i]){
						    ...
						    break;
							case "15599": //product ids can be found on the webstore page
								player.isDonor = true;
								if (player.isIronMan == true) {
								    getAppearence().setTitle(5612);
								}
								if (player.isIronMan != true) {
								    getAppearence().setTitle(5613);
								...
								World.sendWorldMessage(
									"<img=9><col=009fff> " +  player.getDisplayName() + " just donated!", false);
							break;
							case "15600": //product ids can be found on the webstore page
								player.isExtrDonor = true;
								if (player.isIronMan == true) {
								    getAppearence().setTitle(6611);
								}
								if (player.isIronMan != true) {
								...
								World.sendWorldMessage(
									"<img=8><col=7F00FF> " +  player.getDisplayName() + " just donated!", false);
							break;
						}
					}
			}
