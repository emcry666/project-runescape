...
public class VoragoChallenge extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		...
		if (VoragoHandler.getPlayersCount() == 0) {
			...
			sendNPCDialogue(17161, CONFUSED, "Very well. The current instance contains " + VoragoHandler.getPlayersCount() + " player(s)."
					+ " It is owned by "+ World.ChallengerName + ". Would you like to join?");
			stage = 85;
		}
		} else if (stage == 6) {
			end();
			...
			sendNPCDialogue(17161, NORMAL, "You have defeated me in the past. Consider yourself worthy.");
				...
				sendNPCDialogue(17161, NORMAL, "To prepare for what is to come, I must fight at the peak of my strength. "
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				for (Player p : players) {
					...
					if (count == 1) {
						...
						p.applyHit(new Hit(p, (5000/players.size()), HitLook.REGULAR_DAMAGE));//Should be 50000 but I don't have enough players
