...
public class SlayerManager implements Serializable {
	...
	public transient final static int BUY_INTERFACE = 1308, ABILITIES_INTERFACE = 1308, ASSIGNMENT_INTERFACE = 1308, AQUANTITIES = 0, QUICK_BLOWS = 1, ICE_STYKE = 2, BROAD_TIPS = 3, RING_OF_SLAYING = 4,
			SLAYER_HELMET = 5;
	...
	private void sendPoints(int interfaceId) {
		player.getPackets().sendIComponentText(interfaceId, 436, "" + slayerPoints);
	}
	...
	public void handleRewardButtons(int interfaceId, int componentId) {
		if (interfaceId == BUY_INTERFACE) {
			...
			} else if (componentId == 429) {
			    if (player.getSlayerManager().learnedAbilities[5] == false) 
			    {
				if (removePoints(400)) {
				    player.sendMessage("to make a full slayer helment by combining a spiny helmet, a pair of earmuffs, a nose peg, a face mask, and a black mask");
				    player.getSlayerManager().learnedAbilities[5] = true;
				}
			    } else {
				player.sendMessage("You are already wise enough to learn this spell.");
			    }
			}
