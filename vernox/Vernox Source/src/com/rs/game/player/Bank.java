...
public class Bank implements Serializable {
	...
	checkPinReset();
	boolean verified = pin == -1 || this.verified;
	if (!verified)
	    openPin(type);
	return verified;
    }
    ...
    public void switchItem(int fromSlot, int toSlot, int fromComponentId, int toComponentId) {
 	if (toSlot == 65535 || toSlot == 1 || toSlot == 0) {
 	    ...
 	    int toTab = toComponentId >= 253 ? 8 - (261 - toComponentId) : ((toComponentId - 84) / 8) - 7;
 	    if (toTab < 0 || toTab > 9)
 		return;
	...
	return player.isDonor == true ? 4 : 1;
    }
	...
	player.getInterfaceManager().setInterface(true, 762, 112, 1463);
