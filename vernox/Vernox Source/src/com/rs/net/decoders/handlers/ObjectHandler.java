...
import com.rs.game.npc.kalphite.KalphiteHiveHandler;
...
import com.rs.game.player.actions.divination.ConvertWisp;
...
import com.rs.game.player.content.shortcuts.Keldagrim;
...
public final class ObjectHandler {
	...
	if (object.getId() == 91557)
	{
	    player.setNextWorldTile(new WorldTile(4501, 6283, 1));
	}
	
	switch(object.getId()) // slayer tower
	{
	case 52843:
	    if (player != null)
	    {
		...
		@Override
		public void run() {
		    if (player.getY() == 3731 && player.getX() == 3440)
		    {
			player.faceObject(object);
			player.setNextWorldTile(new WorldTile(3439, 3731, 0));
		    }	    
		    else if (player.getY() == 3731 && player.getX() == 3439)
		    {
			player.faceObject(object);
			player.setNextWorldTile(new WorldTile(3440, 3731, 0));
		    }
		}
	    }, true));
	    return;
	...
	case 52844:
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
		    @Override
		    public void run() {
			if (player.getX() == 3441 && player.getY() == 3737)
			{
			    player.faceObject(object);
			    player.setNextWorldTile(new WorldTile(3441, 3738, 0));
			}	    
			else if (player.getX() == 3441 && player.getY() == 3738)
			{
			    player.faceObject(object);
			    player.setNextWorldTile(new WorldTile(3441, 3737, 0));
			}
		    }
		}, true));
		return;
	...
	case 52846:
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
		    @Override
		    public void run() {
			if (player.getX() == 3459 && player.getY() == 3731)
			{
			    player.faceObject(object);
			    player.setNextWorldTile(new WorldTile(3460, 3731, 0));
			}	    
			else if (player.getX() == 3460 && player.getY() == 3731)
			{
			    player.faceObject(object);
	...
	case 82728:
	    ...
	    if (x == 3423 && y == 3539)
	    {
			...
			@Override
			public void run() {
			    if (player.getY() == 3538 && player.getX() == 3423)
			    {
			    player.faceObject(object);
			    player.useStairs(-1, new WorldTile(3423, 3540, 0), 0, 1);
			    }	    
			    else if (player.getY() == 3540 && player.getX() == 3423)
			    {
				player.useStairs(-1, new WorldTile(3423, 3538, 0), 0, 1);
	...
	player.setRouteEvent(new RouteEvent(object, new Runnable() {
		...
		if (TrapAction.isTrap(player, object, id) || TrapAction.isTrap(player, object))
		    ...
		    {
			    player.getPackets().sendCSVarInteger(3711, w.getMemoryId());
			    player.getPackets().sendCSVarInteger(3713, w.getMemoryId());
		   ...
		   //player.getDungManager().enterDungeon(true);
		    player.sendMessage("Dungeoneering is Disabled!");
		...
		// polypore dungon
			...
			WorldTasksManager.schedule(new WorldTask() {
			    ...
			    public void run() {
				int value = player.getVarsManager().getBitValue(10232);
				player.getVarsManager().sendVar(10232, value - 1);
				if (value == 1)
				    stop();
			    }
			}, 9, 9);
		...
		// rock living caverns
			...
			public void run() {
				...
				public void run() {
					...
					// WorldObject(45078, 0, 3, 3651, 5123,
					// 0), new
					// Animation(12220));
				    } else if (count == 5) {
					player.unlock();
		...
		// start of varrock dungeon
		    ...
		    if (player.getX() == 3235 && player.getY() == 2858) {
			KalphiteHiveHandler.KKEntrance(player, object);
		    }
		} else if (id == 82021) {
		    if (player.getX() == 2949 && player.getY() == 1659) {
			KalphiteHiveHandler.ExitKK(player, object);
		    }
		} else if (id == 82022) {
		    if (player.getX() == 2961 && player.getY() == 1658) {
			KalphiteHiveHandler.BlockedTunnel(player, object);
		    }
		...
		if (Settings.DEBUG)
		    player.getPackets()
			    .sendGameMessage("clicked 1 at object id : " + id + ", " + object.getX() + ", "
				    + object.getY() + ", " + object.getPlane() + ", " + object.getType() + ", "
				    + object.getRotation() + ", " + object.getDefinitions().name + ", "
				    + object.getDefinitions().getSizeX() + ", " + object.getDefinitions().getSizeY());
	    }
	}));
    }
	...
	player.setRouteEvent(new RouteEvent(object, new Runnable() {
		...
		player.stopAll();
		player.faceObject(object);
		   ConvertWisp.handleOption2(player, object);
		   ThievingStalls.Steal(player, object);
		   Keldagrim.handleObjects(player, object);
...
//		    player.getActionManager().setAction(new ConvertAction(1));
		...
		if (Settings.DEBUG)
		    player.getPackets().sendGameMessage("clicked 2 at object id : " + id + ", " + object.getX() + ", "
	...
	player.setRouteEvent(new RouteEvent(object, new Runnable() {
		...
		player.stopAll();
		player.faceObject(object);
		ConvertWisp.handleOption3(player, object);
...
//		    player.getActionManager().setAction(new ConvertAction(6));
		...
		if (Settings.DEBUG)
		    player.getPackets().sendGameMessage("cliked 3 at object id : " + id + ", " + object.getX() + ", "
			    + object.getY() + ", " + object.getPlane() + ", ");
	    }
	}));
    }
	...
	player.setRouteEvent(new RouteEvent(object, new Runnable() {
		...
		if (Settings.DEBUG)
		    player.getPackets().sendGameMessage("cliked 4 at object id : " + id + ", " + object.getX() + ", "
			    + object.getY() + ", " + object.getPlane() + ", ");
	    }
	}));
    }
	...
	player.setRouteEvent(new RouteEvent(object, new Runnable() {
		...
		if (Settings.DEBUG)
		    player.getPackets().sendGameMessage("cliked 5 at object id : " + id + ", " + object.getX() + ", "
			    + object.getY() + ", " + object.getPlane() + ", ");
	    }
	}));
    }
	...
	player.setRouteEvent(new RouteEvent(object, new Runnable() {
		...
		if (itemId >= 1438 && itemId <= 1450) {
			...
			ForgingBar bar1 = Smithing.ForgingBar.getBar(itemId);
			player.getDialogueManager().startDialogue("SmithingD", object, bar1);
			break;
		    case "furnace":
		    case "lava furnace":
			if (item.getId() == 2357) {// TODO implement accesory
			    ...
			    if (bars != null) { // else nothing interestin
						// happens
