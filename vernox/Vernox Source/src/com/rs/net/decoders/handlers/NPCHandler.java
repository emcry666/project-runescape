...
import com.rs.game.player.actions.divination.HarvestAction;
...
public class NPCHandler {
	...
	if (npc.getId() == 20282) {
	    player.getDialogueManager().startDialogue("CrystalTool", npc.getId());
	}
	if (npc.getId() == 4250) {
	    player.setRouteEvent(new RouteEvent(npc, new Runnable() {
		@Override
	...
	if (npc.getDefinitions().name.toLowerCase().equals("grand exchange clerk")) {
	    ...
	    {
		player.getPackets().sendGameMessage("<col=FF0000>You cannot acces shops as an ironman</col>");
	    }
	    else
	    {
	    player.setRouteEvent(new RouteEvent(npc, new Runnable() {
	...
	if (npc.getId() == 14362)
	{
	    player.getDialogueManager().startDialogue("AgilityStore", npc.getId());
	}
...
//		Wisps wisps = Wisps.canHarvest(npc);
//		  if (wisps != null) {
...
//		}
		...
		if (npc.getId() == 15775) {
		    player.getDialogueManager().startDialogue("GlobalTeleporter", npc.getId());
		}
		
		if (npc.getId() == 961) {
		    player.getDialogueManager().startDialogue("HealerSurgeon", npc.getId());
		}
		...
		if (npc.getId() == 13930)
		{
		    ...
		    {
		    player.getDialogueManager().startDialogue("GlobalShop", npc.getId());
		    }
		}
		...
		{
		    if (player.getSkills().getLevel(Skills.SLAYER) < 70 && player.getCombatLevel() < 100)
		    {
			player.getDialogueManager().startDialogue("GrimReq");
		    }
		    else
		    {
		    player.getDialogueManager().startDialogue("GrimD");
		    }
		}
		...
		else if (npc.getId() == 15772) {
		    if (player.getEquipment().wearingArmour()){
			player.sendMessage("You may not wear any items to perform this action!");
		    } else {
	...
	player.setRouteEvent(new RouteEvent(npc, new Runnable() {
		...
		if (shipAttributes != null) {
		    ...
		    player.getDialogueManager().startDialogue("DungeonShop");
		    player.sendMessage("You currently have " + player.getDungTokens() + " dungeoneering tokens.");
		}
		else if (npc.getId() == 5915)
		    player.getDialogueManager().startDialogue("ClaimClanItem", npc.getId(), 20709);
		else if (npc.getId() == 13633)
