...
public class InventoryOptionsHandler {
	...
	public static void handleItemOption2(final Player player, final int slotId, final int itemId, Item item) {
		...
		if (Firemaking.isFiremaking(player, itemId))
			...
			player.getSlayerManager().checkKillsLeft();
		    	...
		    	player.getPackets().sendGameMessage("Your current assignment is " +player.getTask().getName() + "; only " +player.getTask().getTaskAmount()+ " left to go");
		}
	...
	public static void handleItemOption1(final Player player, final int slotId, final int itemId, Item item) {
		...
		Weaves weave = WeaveAction.Weaves.getEnergy(itemId);
		for(Weaves w : Weaves.values())
		{
		   for(int i = 0; i < w.getEnergy().length; i++)
		   {
		       if(item.getId() == w.getEnergy()[i].getId())
		       {
			   player.getDialogueManager().startDialogue("DivinationD", weave);
		       }
		   }
		}
		if (itemId == 6099) {
		    item.getDefinitions().setName("Global Teleports");
		...
		//div not being used
			...
			player.getPackets().sendGameMessage("You place a portable range infront of you.");
			WorldTasksManager.schedule(new WorldTask() {
			    int ticks;
			    @Override
			    public void run() {
			...
			if(ticks == 1){
				...
				final WorldObject divinerune = new WorldObject(89768,10, 0, player.getX() + 1, player.getY(), player.getPlane()); // object spawning
				player.faceObject(divinerune); // forces player to face where he is putting the object
				World.spawnTemporaryObject(divinerune, 60000); // dont touch
				stop();
			}
