		...
		if (opcode == WALKING_PACKET || opcode == MINI_WALKING_PACKET) {
			...
			player.setRouteEvent(new RouteEvent(p2, new Runnable() {
				...
				public void run() {
					...
					{
					    player.getPackets().sendGameMessage("Ironmen cannot trade.");
						return;  
					}
					if (p2.isIronMan == true)
					{
					    player.getPackets().sendGameMessage("Ironmen cannot trade.");
						return;  
					}
			...
			if (item == null)
				return;
			if(player.isIronMan == true) {
				if(item.getOwner() != player.getUsername() && item.getOwner() != null ) {
					player.getPackets().sendGameMessage("You are not allowed to pick up others items while on Ironman Mode.");
					return;
				}
			}
			if (forceRun)
		...
		if (opcode == PING_PACKET) {
			...
			if ((fromInterfaceId == Inventory.INVENTORY_INTERFACE || fromInterfaceId == Inventory.INVENTORY_INTERFACE_2) && (toInterfaceId == Inventory.INVENTORY_INTERFACE || toInterfaceId == Inventory.INVENTORY_INTERFACE_2)) {
				...
				if (toSlot >= player.getInventory().getItemsContainerSize()
				    ...
				    return;
				player.getInventory().switchItem(fromSlot, toSlot);
			    } else if (fromComponentId == 7 && toComponentId != 7) {

			    } else if (fromComponentId == 215 && toComponentId == 7) {
